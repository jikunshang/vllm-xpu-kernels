...
void expandInputRowsKernel(
  ...
  int64_t expanded_dest_row = item_ct1.get_group(2);
  ...
  if constexpr (ALIGN_BLOCK_SIZE) {
    // load g2s
    for (int idx = item_ct1.get_local_id(2); idx < num_local_experts + 1;
         idx += item_ct1.get_local_range(2)) {
      smem_expert_first_token_offset[idx] =
          /*
          DPCT1098:294: The '*' expression is used instead of the __ldg call.
    ...
    */
    item_ct1.barrier();
    int lane_idx = item_ct1.get_local_id(2) & 31;
...
#pragma unroll 1
      for (int eidx = 1; eidx <= sycl::min(expert_id, num_local_experts);
           eidx++) {
    ...
    expanded_dest_row = dpct::select_from_sub_group(item_ct1.get_sub_group(),
                                                    expanded_dest_row, 0);
  }
  ...
  if (!CHECK_SKIPPED || item_ct1.get_group(2) < *num_dest_rows) {
    ...
    int64_t const start_offset = item_ct1.get_local_id(2);
    int64_t const stride = item_ct1.get_local_range(2);
...
void expandInputRowsKernelLauncher(
    ...
    int num_local_experts, const int& align_block_size,
  ...
  dpct::kernel_launcher::launch(
      func, blocks, threads, smem_size, stream, unpermuted_input,
      permuted_output, unpermuted_scales, sorted_experts,
...
constexpr static U arrayConvert(T const& input) {
...
void finalizeMoeRoutingKernel(
    ...
    int64_t const* num_valid_ptr, const sycl::nd_item<3> &item_ct1) {
  assert(orig_cols % 4 == 0);
  int64_t const original_row = item_ct1.get_group(2);
  int64_t const num_rows = item_ct1.get_group_range(2);
...
void finalizeMoeRoutingKernelLauncher(
  ...
  dpct::kernel_launcher::launch(kernel, blocks, threads, 0, stream,
                                expanded_permuted_rows,
                                reduced_unpermuted_output, scales,
                                expanded_source_row_to_expanded_dest_row,
                                expert_for_source_row, cols, k, num_valid_ptr);
}
