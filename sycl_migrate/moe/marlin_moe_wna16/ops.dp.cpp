...
void MarlinDefault_wrapper(
    ...
    const int* __restrict g_idx, const int32_t* __restrict sorted_token_ids_ptr,
    const int32_t* __restrict expert_ids_ptr,
    const int32_t* __restrict num_tokens_past_padded_ptr,
    const float* __restrict topk_weights_ptr, int top_k, bool mul_topk_weights,
    bool is_ep, int num_groups, int prob_m, int prob_n, int prob_k, int* locks,
  ...
  queue.parallel_for(nr, [=](sycl::nd_item<3> item_ct1) {
    MarlinDefault(A, B, C, C_tmp, scales_ptr, scale2_ptr, zp_ptr, g_idx,
                  ...
                  locks, use_atomic_add, use_fp32_reduce, max_shared_mem);
  });
};
...
void permute_cols_kernel(sycl::int4 const* __restrict__ a_int4_ptr,
                         ...
                         sycl::int4* __restrict__ out_int4_ptr,
                         const int32_t* __restrict__ sorted_token_ids_ptr,
                         const int32_t* __restrict__ expert_ids_ptr,
  ...
  auto read_moe_block_data = [&](int block_id, const sycl::nd_item<3> &item_ct1) {
    block_num_valid_tokens = moe_block_size;
    sycl::int4* tmp_block_sorted_ids =
        reinterpret_cast<sycl::int4*>(block_sorted_ids);
    for (int i = 0; i < moe_block_size / 4; i++) {
      tmp_block_sorted_ids[i] = ((
          sycl::int4*)sorted_token_ids_ptr)[block_id * moe_block_size / 4 + i];
    }
  ...
  for (int index = item_ct1.get_group(2); index < num_moe_blocks;
       index += item_ct1.get_group_range(2)) {
...
typedef struct dpct_type_143306 {
  int thread_k;
  int thread_n;
  int num_threads;
} thread_config_t;
...
typedef struct dpct_type_288298 {
  int blocks_per_sm;
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* s,
               ...
               int dev, dpct::queue_ptr stream, int thread_k, int thread_n,
  ...
  /*
  DPCT1019:297: local_mem_size in SYCL is not a complete equivalent of
  ...
  */
  ...
  // avoid ">>>" being formatted to "> > >"
  // clang-format off
  dpct::kernel_launcher::launch(kernel, blocks, num_threads, max_shared_mem, stream, A_ptr, B_ptr, C_ptr, C_tmp_ptr, s_ptr, s2_ptr, zp_ptr, g_idx_ptr, sorted_token_ids_ptr, expert_ids_ptr, num_tokens_past_padded_ptr, topk_weights_ptr, top_k, mul_topk_weights, is_ep, num_groups, prob_m, prob_n, prob_k, locks, use_atomic_add, use_fp32_reduce, max_shared_mem);
  // clang-format on
}
