...
#include "type_convert.dp.hpp"
#include "quantization/fp8/common.dp.hpp"
...
void rms_norm_static_fp8_quant_kernel(
    ...
    const sycl::nd_item<3> &item_ct1, float &s_variance,  &reduceStore) {
  float variance = 0.0f;
...
std::enable_if_t<(width > 0) && _typeConvert<scalar_t>::exists>
fused_add_rms_norm_static_fp8_quant_kernel(
  ...
  using BlockReduce = cub::BlockReduce<float, 1024>;
  typename BlockReduce::TempStorage reduceStore;
  variance = BlockReduce(reduceStore).Reduce(variance, cub::Sum{}, blockDim.x);
...
void fused_add_rms_norm_static_fp8_quant(
  ...
  dpct::dim3 block(std::min(hidden_size, max_block_size));
  const at::cuda::OptionalCUDAGuard device_guard(device_of(input));
  const dpct::queue_ptr stream = at::cuda::getCurrentCUDAStream();
