...
namespace vllm {
/*
DPCT1009:30: SYCL reports errors using exceptions and does not use error codes.
 ...
 public:
  ...
  char* open_ipc_handle(const void* ipc_handle) try {
    ...
    if (new_handle) {
      char* ipc_ptr;
      /*
      DPCT1030:29: SYCL currently does not support inter-process communication
  ...
  std::pair<std::string, std::vector<int64_t>> get_graph_buffer_ipc_meta() try {
    ...
    for (int i = 0; i < num_buffers; i++) {
      ...
      // address
      /*
      DPCT1007:31: Migration of cuPointerGetAttribute is not supported.
      */
      if (cuPointerGetAttribute(&base_ptr, rangeStartAddrAttr,
                                (dpct::device_ptr)ptr) != 0)
        throw std::runtime_error("failed to get pointer attr");
      /*
      DPCT1030:40: SYCL currently does not support inter-process communication
      (IPC) operations. You may need to rewrite the code.
      */
  ...
  void register_buffer(void** ptrs) try {
    ...
    /*
    DPCT1064:41: Migrated cudaMemcpy call is used in a macro/template definition
  ...
  void register_graph_buffers(
    ...
    /*
    DPCT1064:42: Migrated cudaMemcpy call is used in a macro/template definition
    and may not be valid for all macro/template uses. Adjust the code.
    */
    CUDACHECK(DPCT_CHECK_ERROR(dpct::get_in_order_queue()
  ...
  void allreduce(dpct::queue_ptr stream, T* input, T* output, int size,
    ...
    RankData* ptrs;
    /*
    DPCT1119:32: Migration of cudaStreamCaptureStatus is not supported, please
    try to remigrate with option: --use-experimental-features=graph.
    */
    cudaStreamCaptureStatus status;
    /*
    DPCT1119:33: Migration of cudaStreamIsCapturing is not supported, please try
    to remigrate with option: --use-experimental-features=graph.
    */
    CUDACHECK(cudaStreamIsCapturing(stream, &status));
    /*
    DPCT1119:34: Migration of cudaStreamCaptureStatusActive is not supported,
...
#undef KL
  ...
  ~CustomAllreduce() try {
    for (auto [_, ptr] : ipc_handles_) {
      /*
      DPCT1030:43: SYCL currently does not support inter-process communication
      (IPC) operations. You may need to rewrite the code.
      */
