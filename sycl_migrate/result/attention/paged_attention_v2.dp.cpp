...
*/
/*
DPCT1049:14: The work-group size passed to the SYCL kernel may exceed the limit.
To get the device limit, query info::device::max_work_group_size. Adjust the
work-group size if needed.
*/
#define LAUNCH_PAGED_ATTENTION_V2(HEAD_SIZE)                                   \
   stream->submit([&](sycl::handler& cgh) {                                    \
      ...
      auto seq_lens_ptr_ct9 = seq_lens_ptr;                                    \
      ...
      auto k_scale_ptr_ct15 = k_scale_ptr;                                     \
      auto v_scale_ptr_ct16 = v_scale_ptr;                                     \
      auto tp_rank_ct17 = tp_rank;                                             \
      auto blocksparse_local_blocks_ct18 = blocksparse_local_blocks;           \
      auto blocksparse_vert_stride_ct19 = blocksparse_vert_stride;             \
      auto blocksparse_block_size_ct20 = blocksparse_block_size;               \
      auto blocksparse_head_sliding_step_ct21 = blocksparse_head_sliding_step; \
                                                                               \
      cgh.parallel_for(                                                        \
          ...
          [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(32)]] {   \
             vllm::paged_attention_v2_kernel<                                  \
                 ...
                 seq_lens_ptr_ct9, max_num_blocks_per_seq_ct10,                \
                 alibi_slopes_ptr_ct11, q_stride_ct12, kv_block_stride_ct13,   \
                 kv_head_stride_ct14, k_scale_ptr_ct15, v_scale_ptr_ct16,      \
                 tp_rank_ct17, blocksparse_local_blocks_ct18,                  \
                 blocksparse_vert_stride_ct19, blocksparse_block_size_ct20,    \
                 blocksparse_head_sliding_step_ct21, item_ct1,                 \
                 dpct_local_acc_ct1                                            \
   ...
   stream->submit([&](sycl::handler& cgh) {                                    \
      ...
      auto seq_lens_ptr_ct4 = seq_lens_ptr;                                    \
      ...
      cgh.parallel_for(                                                        \
          ...
          [=](sycl::nd_item<3> item_ct1) [[sycl::reqd_sub_group_size(32)]] {   \
             vllm::paged_attention_v2_reduce_kernel<T, HEAD_SIZE, NUM_THREADS, \
                 ...
                 tmp_out_ptr_ct3, seq_lens_ptr_ct4, max_num_partitions_ct5,    \
                 item_ct1,                                                     \
                 dpct_local_acc_ct1                                            \
                     .get_multi_ptr<sycl::access::decorated::no>()             \
                     .get(),                                                   \
...
void paged_attention_v2_launcher(
  ...
  /*
  DPCT1083:15: The size of local memory in the migrated code may be different
  ...
  */
  ...
  dpct::dim3 grid(num_heads, num_seqs, max_num_partitions);
  int shared_mem_size = std::max(logits_size, outputs_size);
  // For paged attention v2 reduce kernel.
  dpct::dim3 reduce_grid(num_heads, num_seqs);
  /*
  DPCT1083:13: The size of local memory in the migrated code may be different
  from the original code. Check that the allocated memory size in the migrated
  code is correct.
  */
