...
int64_t save_blocks(int* block_offset, int64_t range_start,
...
void convert_vertical_slash_indexes_kernel(
    ...
    bool causal  , const sycl::nd_item<3> &item_ct1// True for intra, False for succ
) {
  const int batch_idx = item_ct1.get_group(1);
  const int head_idx = item_ct1.get_group(2);
  const int group_idx = item_ct1.get_group(0);

  int64_t q_seqlen = q_seqlens[batch_idx];
  int64_t kv_seqlen = kv_seqlens[batch_idx];
  int64_t block_idx_m =
      group_idx * item_ct1.get_local_range(2) + item_ct1.get_local_id(2);
  ...
  if (causal) {
    ...
    s_idx = sycl::max((kv_seqlen - q_seqlen) + end_m - s_idx, BLOCK_SIZE_M);
  } else {
    while (s_idx >= end_m + kv_seqlen && s < NNZ_S) {
      s_idx = slash_indexes[s++];
    }
    if (s_idx > end_m + kv_seqlen) has_slash = false;
    s_idx = sycl::max(kv_seqlen + end_m - s_idx, BLOCK_SIZE_M);
  }
  ...
  while (1) {
    if (v_idx < range_end) {
      ...
      if ((s < NNZ_S && causal) ||
        ...
        if (causal)
          s_idx = sycl::max((kv_seqlen - q_seqlen) + end_m - slash_indexes[s++],
                            BLOCK_SIZE_M);
        else
          s_idx =
              sycl::max(kv_seqlen + end_m - slash_indexes[s++], BLOCK_SIZE_M);
      } else {
...
void convert_vertical_slash_indexes_64x64(
  ...
  const dpct::dim3 dimBlock(N_THREADS);
  const dpct::dim3 dimGrid(N_HEADS, BATCH_SIZE,
                           (N_ROWS + N_THREADS - 1) / N_THREADS);
   dpct::get_in_order_queue().parallel_for(
       sycl::nd_range<3>(dimGrid * dimBlock, dimBlock),
       [=](sycl::nd_item<3> item_ct1) {
          convert_vertical_slash_indexes_kernel(
              q_seqlens, kv_seqlens, vertical_indexes, slash_indexes,
              block_count, block_offset, column_count, column_index, N_HEADS,
              N_ROWS, BLOCK_SIZE_M, BLOCK_SIZE_N, NNZ_V, NNZ_S, causal,
              item_ct1);
       });
}
   ...
   dpct::get_in_order_queue().parallel_for(
       ...
       [=](sycl::nd_item<3> item_ct1) {
          convert_vertical_slash_indexes_kernel_mergehead(
              q_seqlens, kv_seqlens, vertical_indexes, slash_indexes,
              per_head_vertical_topkv, per_head_slash_topkv, block_count,
              block_offset, column_count, column_index, N_HEADS, N_ROWS,
              BLOCK_SIZE_M, BLOCK_SIZE_N, NNZ_V, NNZ_S, causal, item_ct1);
       });
}
