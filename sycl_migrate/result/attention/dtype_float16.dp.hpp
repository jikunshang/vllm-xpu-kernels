...
#ifndef USE_ROCM
  uint32_t b;
   b = sycl::vec<uint16_t, 2>({a, a}).template as<sycl::vec<uint32_t, 1>>()[0];
  return b;
#else
  union {
    uint32_t u32;
...
inline float half_to_float(uint16_t h) {
  float f;
#ifndef USE_ROCM
   f = sycl::vec<uint16_t, 1>(h)
           .template as<sycl::vec<sycl::half, 1>>()
           .template convert<float, sycl::>()
           .x();
#else
...
#ifndef USE_ROCM
  uint16_t lo, hi;
   lo = sycl::vec<uint32_t, 1>(v).template as<sycl::vec<uint16_t, 2>>()[0];
   hi = sycl::vec<uint32_t, 1>(v).template as<sycl::vec<uint16_t, 2>>()[1];
...
inline uint32_t fma(uint32_t a, uint32_t b, uint32_t c) {
  uint32_t d;
#ifndef USE_ROCM
   d = sycl::fma(sycl::vec<uint32_t, 1>(a).template as<sycl::half2>(),
                 sycl::vec<uint32_t, 1>(b).template as<sycl::half2>(),
                 sycl::vec<uint32_t, 1>(c).template as<sycl::half2>())
...
inline float to_float(uint16_t u) { return half_to_float(u); }

inline sycl::float2 to_float(uint32_t u) { return half2_to_float2(u); }
...
// Zero-out a variable.
inline void zero(uint16_t& dst) { dst = uint16_t(0); }
