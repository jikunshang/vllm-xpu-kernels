...
moe_wna16_gemm_kernel exceeds 128 bytes and may cause high register pressure.
Consult with your hardware vendor to find the total register size available and
adjust the code, or use smaller sub-group size to avoid high register pressure.
*/
void moe_wna16_gemm_kernel(
    ...
    bool mul_topk_weight, const sycl::nd_item<3>& item_ct1,
    uint8_t* dpct_local) {
  ...
  if constexpr (std::is_same<scalar_t, nv_bfloat16>::value) {
    ...
    if (item_ct1.get_group(2) * BLOCK_SIZE_M >= num_tokens_post_pad[0]) return;

    const int32_t offset_n =
        item_ct1.get_group(1) * BLOCK_SIZE_N + item_ct1.get_local_id(2);
    const int32_t offset_k = item_ct1.get_group(0) * BLOCK_SIZE_K;

    const int32_t expert_id = expert_ids[item_ct1.get_group(2)];

    int32_t num_valid_tokens = 0;
    auto block_input_tmp = (uint16_t*)dpct_local;
    ...
    for (int m = 0; m < BLOCK_SIZE_M; m++) {
      const int32_t offset_m = item_ct1.get_group(2) * BLOCK_SIZE_M + m;
      ...
      if (expert_id != -1) {
        ...
        for (int i = 0; i < k_per_thread; i++) {
          int k = BLOCK_SIZE_N * i + item_ct1.get_local_id(2);
          ...
          if constexpr (bit == 4) {
            // [0, 4, 1, 5, 2, 6, 3, 7]
            int8_t order = (item_ct1.get_local_id(2) % 2) * 4 +
                           ((item_ct1.get_local_id(2) % 8) / 2);
            origin_k =
                BLOCK_SIZE_N * i + item_ct1.get_local_id(2) / 8 * 8 + order;
          } else {
            // [0, 2, 1, 3]
            int8_t order = (item_ct1.get_local_id(2) % 2) * 2 +
                           ((item_ct1.get_local_id(2) % 4) / 2);
            origin_k =
                BLOCK_SIZE_N * i + item_ct1.get_local_id(2) / 4 * 4 + order;
          }

          origin_k += token_index / top_k * size_k +
                      item_ct1.get_group(0) * BLOCK_SIZE_K;
    ...
    */
    item_ct1.barrier();
    if (item_ct1.get_local_id(2) >= BLOCK_SIZE_N || offset_n >= size_n) return;
    ...
    sycl::float4* expert_qweight_tmp_float4 =
        reinterpret_cast<sycl::float4*>(expert_qweight_tmp);
    ...
    if constexpr (GROUPS == 1) {
      ...
      *expert_scales_groups_tmp =
      ...
      sycl::float2* expert_scales_groups_tmp =
          reinterpret_cast<sycl::float2*>(expert_scales_groups);
      *expert_scales_groups_tmp = reinterpret_cast<const sycl::float2*>(
          expert_scales)[scales_offset_tmp];
    } else if constexpr (GROUPS == 8) {
      sycl::float4* expert_scales_groups_tmp =
          reinterpret_cast<sycl::float4*>(expert_scales_groups);
      *expert_scales_groups_tmp = reinterpret_cast<const sycl::float4*>(
          expert_scales)[scales_offset_tmp];
    }
    ...
    for (int tmp_k = 0; tmp_k < BLOCK_SIZE_K / pack_factor; tmp_k++) {
      ...
      if (tmp_k % 4 == 0) {
        *expert_qweight_tmp_float4 = reinterpret_cast<const sycl::float4*>(
      ...
      if (tmp_k % (group_size / pack_factor) == 0) {
        ...
        if (has_zp) {
          ...
          if constexpr (bit == 4) {
            qzero = (qzero >> ((item_ct1.get_local_id(2) % 2) * 4)) & 0xF;
          }
...
#pragma unroll
    ...
    for (int m = 0; m < num_valid_tokens; ++m) {
      const int32_t token_index =
          sorted_token_ids[item_ct1.get_group(2) * BLOCK_SIZE_M + m];
      if (mul_topk_weight) {
...
void run_moe_wna16_gemm(const scalar_t* input, scalar_t* output,
  ...
  dpct::kernel_launcher::launch(
      kernel, gridDim, blockDim, shared_mem_size, stream, input, output,
      b_qweight, b_scales, b_qzeros, topk_weights, sorted_token_ids, expert_ids,
      num_tokens_post_pad, num_experts, group_size, top_k, size_m, size_n,
      size_k, BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K, has_zp,
      mul_topk_weight);
}
