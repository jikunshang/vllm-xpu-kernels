...
void __get_group_gemm_starts(
    ...
    const int K, const int N, const sycl::nd_item<3> &item_ct1) {
  int64_t expert_id = item_ct1.get_local_id(2);
...
void run_fp4_blockwise_scaled_group_mm(
  ...
  const dpct::queue_ptr stream = at::cuda::getCurrentCUDAStream(a.get_device());
