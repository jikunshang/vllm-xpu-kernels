...
struct GmemTile_W8A16_PerC_MtilexNtilex32_multistage_SM8x_SplitK {
  ...
  GmemTile_W8A16_PerC_MtilexNtilex32_multistage_SM8x_SplitK(
    ...
    this_block_A_base_ptr = params.A_ptr +
                            item_ct1.get_group(2) * Mtile * params.K +
                            item_ct1.get_group(0) * params.SplitK;
    // here B is rearranged as N32K16 order, i.e. 4 continuous N-direction
    // 8(N)x16(K) size data blocks are packed together
    this_block_B_base_ptr = params.B_ptr +
                            item_ct1.get_group(1) * Ntile * params.K +
                            item_ct1.get_group(0) * params.SplitK * 4;

    const auto lane_id = item_ct1.get_local_id(2) % WARP_SIZE;
    ...
    const auto Aldg_row_base_idx = item_ct1.get_local_id(2) / 4;
    Aldg_col_idx = (item_ct1.get_local_id(2) % 4) * LDG_ELEMENT_CNT_A;
    ...
    // * 128(col) per iter
    Bldg_col_idx = (item_ct1.get_local_id(2) % 8) * LDG_ELEMENT_CNT_B;
    const auto Bldg_row_base_idx = item_ct1.get_local_id(2) / 8;
  ...
  #pragma unroll
    for (int i = 0; i < (Mtile + M_SIZE_ONE_LOAD - 1) / M_SIZE_ONE_LOAD; ++i) {
      auto m_idx = item_ct1.get_group(2) * Mtile + Aldg_row_base_idx +
                   i * M_SIZE_ONE_LOAD;
      if (m_idx < params.M) {
        A_ldg_guard |= (1u << i);
      }
  ...
  #pragma unroll
    for (int i = 0; i < (Ntile + N_SIZE_ONE_LOAD - 1) / N_SIZE_ONE_LOAD; ++i) {
      auto n_idx = item_ct1.get_group(1) * Ntile +
                   (Bldg_row_base_idx / 8) * 32 + i * N_SIZE_ONE_LOAD;
      if (n_idx < N_padded) {
        B_ldg_guard |= (1u << i);
      }
    }
  ...
  void ldgsts_first_ktiles(const int& first_k_tile,
                                      const int& k_tiles) {
    // load first k_tile
    // load A
  ...
  void ldgsts(const int& sts_stage_idx) {
...
struct ComputeTile_W8A16_PerC_MtilexNtilex32_multistage_SM8x_SplitK {
  ...
  ComputeTile_W8A16_PerC_MtilexNtilex32_multistage_SM8x_SplitK(
      ...
      : params(k_params),
        ...
        A_smem_stage_stride(A_stage_stride),
        BQ_smem_stage_stride(BQ_stage_stride) {
    warp_id = item_ct1.get_local_id(2) / WARP_SIZE;
    lane_id = item_ct1.get_local_id(2) % WARP_SIZE;
    ...
    if (EnableFuse) {
      this_block_C_base_ptr = params.C_ptr +
                              item_ct1.get_group(2) * Mtile * params.N +
                              item_ct1.get_group(1) * Ntile;
    } else {
      this_block_C_base_ptr = params.C_split_ptr +
                              item_ct1.get_group(0) * params.M * params.N +
                              item_ct1.get_group(2) * Mtile * params.N +
                              item_ct1.get_group(1) * Ntile;
    }
  ...
  #pragma unroll
    ...
    params_n_idx = item_ct1.get_group(1) * Ntile + warp_id * WARP_NTILE +
                   (lane_id / 4) * 4;
  }

  void lds(const int& smem_stage_idx, const int& reg_buf_idx,
    ...
    for (int i = 0; i < WARP_NITER / 2; ++i) {
      ...
      if (has_zp) {
        BF_frag[reg_buf_idx][2 * i][0] =
            BF_frag[reg_buf_idx][2 * i][0] - num2num2(B_zero[i].x);
        BF_frag[reg_buf_idx][2 * i][1] =
            BF_frag[reg_buf_idx][2 * i][1] - num2num2(B_zero[i].x);
      }

      BF_frag[reg_buf_idx][2 * i][0] =
          BF_frag[reg_buf_idx][2 * i][0] * num2num2(B_scale[i].x);
      BF_frag[reg_buf_idx][2 * i][1] =
          BF_frag[reg_buf_idx][2 * i][1] * num2num2(B_scale[i].x);

      cvt_8bx4_to_16bx4_bias128(BQ_frag[reg_buf_idx][2 * i + 1],
                                BF_frag[reg_buf_idx][2 * i + 1]);
      if (has_zp) {
        BF_frag[reg_buf_idx][2 * i + 1][0] =
            BF_frag[reg_buf_idx][2 * i + 1][0] - num2num2(B_zero[i].y);
        BF_frag[reg_buf_idx][2 * i + 1][1] =
            BF_frag[reg_buf_idx][2 * i + 1][1] - num2num2(B_zero[i].y);
      }

      BF_frag[reg_buf_idx][2 * i + 1][0] =
          BF_frag[reg_buf_idx][2 * i + 1][0] * num2num2(B_scale[i].y);
      BF_frag[reg_buf_idx][2 * i + 1][1] =
          BF_frag[reg_buf_idx][2 * i + 1][1] * num2num2(B_scale[i].y);
    }
  }
  ...
  void fused_splitk_reduce(const sycl::nd_item<3> &item_ct1) {
    // need splitk-reduce if enable splitk
    if (item_ct1.get_group_range(0) > 1) {
      auto blk_red_idx = item_ct1.get_group(2) * item_ct1.get_group_range(1) +
      ...
      if (item_ct1.get_local_id(2) == 0) {
        ...
        do {
          ...
          /*
          DPCT1078:190: Consider replacing memory_order::acq_rel with
          ...
          */
          ...
          /*
          DPCT1053:191: Migration of device assembly code is not supported.
          */
          asm volatile("ld.global.cg.b32 %0, [%1];"
                       : "=r"(count)
                       : "l"(red_count_ptr));
        } while (count != item_ct1.get_group(0));
      }
      /*
      ...
      */
      ...
      auto C_tmp_base_offset =
          blk_red_idx * Mtile * Ntile + item_ct1.get_local_id(2) * 4;
  ...
  #pragma unroll
          for (int n_idx = 0; n_idx < WARP_NITER; ++n_idx) {
            ...
            *reinterpret_cast<sycl::int4*>(temp_frag[m_idx][n_idx]) =
                *reinterpret_cast<sycl::int4*>(params.C_tmp_ptr + offset);
          }
        }
  #pragma unroll
        for (int m_idx = 0; m_idx < Mtile / 16; ++m_idx) {
  ...
  #pragma unroll
      ...
      if (item_ct1.get_group(0) != item_ct1.get_group_range(0) - 1) {
  #pragma unroll
        for (int m_idx = 0; m_idx < Mtile / 16; ++m_idx) {
  #pragma unroll
          for (int n_idx = 0; n_idx < WARP_NITER; ++n_idx) {
            int offset =
                C_tmp_base_offset + (m_idx * WARP_NITER + n_idx) * BLOCK * 4;
            /*
            DPCT1053:194: Migration of device assembly code is not supported.
            */
            asm volatile(
        ...
        /*
        DPCT1078:192: Consider replacing memory_order::acq_rel with
        memory_order::seq_cst for correctness if strong memory order
        restrictions are needed.
        */
        sycl::atomic_fence(sycl::memory_order::acq_rel,
        ...
        */
        ...
        if (item_ct1.get_local_id(2) == 0) {
          ...
          dpct::atomic_fetch_compare_inc<
              ...
              red_count_ptr, item_ct1.get_group_range(0));
        }
      }
    }
  }

  void stg(char* smem, const sycl::nd_item<3> &item_ct1) {
    if (EnableFuse) {
      if (item_ct1.get_group(0) != item_ct1.get_group_range(0) - 1) return;
    }
    uint32_t* C_sts_ptr =
  ...
  #pragma unroll
    ...
    */
    ...
    // C_tile lds and stg
    auto m_base_idx = store_c_row_base_idx + item_ct1.get_group(2) * Mtile;
    bool n_guard = (store_c_col_idx + item_ct1.get_group(1) * Ntile) < params.N;
    if (WARP_NTILE == 32) {
      int lds_c_base_offset = warp_id * Mtile * WARP_NTILE +
                              (lane_id / 4) * WARP_NTILE +
                              ((lane_id % 4 + lane_id / 8) % 4) * 8;
      sycl::uint4* C_lds_ptr = reinterpret_cast<sycl::uint4*>(
          smem + lds_c_base_offset * sizeof(FType));
  #pragma unroll
      for (int i = 0; i < (Mtile / 16) * (WARP_NITER / 2); ++i) {
        sycl::uint4 stg_reg = C_lds_ptr[i * 8 * 4];
  ...
  #pragma unroll
      for (int i = 0; i < (Mtile / 16) * (WARP_NITER / 2); ++i) {
        ...
        sycl::uint4 stg_reg = *reinterpret_cast<sycl::uint4*>(
            smem + lds_c_offset * sizeof(FType));
        stg128(stg_reg.x(), stg_reg.y(), stg_reg.z(), stg_reg.w(),
...
void 
    ampere_hgemm_W8A16_perc_f16_f16_MtilexNtilex32_hmma16816_multistage_AN_BTN32K16_CN_splitk_kernel(
        const SM8x_GEMM_W8A16_Splitk_Params<FType, QType> params,
        const sycl::nd_item<3> &item_ct1, char *smem) {
  ...
  auto tb_k_slice =
      item_ct1.get_group(0) * params.SplitK + params.SplitK <= params.K
          ? params.SplitK
          : params.K - item_ct1.get_group(0) * params.SplitK;
  ...
  /*
  DPCT1065:363: Consider replacing sycl::nd_item::barrier() with
  sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
  performance if there is no access to global memory.
  */
  item_ct1.barrier();
  ...
  #pragma unroll
    for (int k_phase_idx = 0; k_phase_idx < 2; k_phase_idx++) {
      ...
      if (k_phase_idx == 1) {
        ...
        */
        /*
        DPCT1065:364: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
  ...
  #pragma unroll
    for (int k_phase_idx = 0; k_phase_idx < 2; k_phase_idx++) {
      ...
      if (k_phase_idx == 1) {
        ...
        */
        /*
        DPCT1065:365: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
  ...
  */
  /*
  DPCT1129:197: The type "SM8x_GEMM_W8A16_Splitk_Params<FType, QType>" is used
  ...
  */
  #define __CALL_IF(MTILE, NTILE, NUM_THREADS, ENABLE_FUSE, HAS_ZP)                                             \
      ...
      stream->submit([&](sycl::handler& cgh) {                                                                  \
        sycl::local_accessor<char, 1> smem_acc_ct1(                                                             \
            sycl::range<1>(4 * smem_size_one_stage), cgh);                                                      \
                                                                                                                \
        auto params_ct0 = params;                                                                               \
                                                                                                                \
        cgh.parallel_for(                                                                                       \
            sycl::nd_range<3>(grid * block, block),                                                             \
            [=](sycl::nd_item<3> item_ct1) {                                                                    \
              ampere_hgemm_W8A16_perc_f16_f16_MtilexNtilex32_hmma16816_multistage_AN_BTN32K16_CN_splitk_kernel< \
                  FType, QType, MTILE, NTILE, 4, NUM_THREADS, ENABLE_FUSE,                                      \
                  HAS_ZP>(                                                                                      \
                  params_ct0, item_ct1,                                                                         \
                  smem_acc_ct1.get_multi_ptr<sycl::access::decorated::no>()                                     \
                      .get());                                                                                  \
            });                                                                                                 \
      });                                                                                                       \
    }
...
void ampere_hgemm_W8A16_perc_f16_f16_MtilexNtilex32_mma16816_multistage_AN_BTN32K16_CN_splitk(
  ...
  dpct::dim3 grid(grid_x, grid_y, grid_z);
  dpct::dim3 block(BLOCK);
  ...
  if (enable_fuse) {
    ...
    CHECK_CUDA(DPCT_CHECK_ERROR(
        stream->memset(red_count, 0, grid_x * grid_y * sizeof(uint32_t))));
...
size_t allspark_qgemm_w8a16_perc_n32k16_ampere_workspace_size(
  ...
  if (enable_fuse) {
    ...
    ws_size = grid_z * m * n * sizeof(sycl::half);
  }
...
void restore_N32_K16_dequantize_rhs_w8a16_perc_kernel(
    const QT* qdata, const FT* scales, const FT* zeros, FT* fdata,
    const int N_32align, const int N, const int K,
    const sycl::nd_item<3> &item_ct1, FT *smem) {
  auto warp_id = item_ct1.get_local_id(2) / 32;
  auto lane_id = item_ct1.get_local_id(2) % 32;
  const auto src_row_idx = item_ct1.get_group(2) * 8 + lane_id / 4;
  const int src_col_idx =
      item_ct1.get_group(1) * 64 * 4 + warp_id * 16 * 4 + (lane_id % 4) * 16;
  const int src_offset = src_row_idx * K * 4 + src_col_idx;
  auto params_nidx = item_ct1.get_group(2) * 32 + (lane_id / 4) * 4;

  QT qval_reg[16];
  const QT* pdata = qdata + src_offset;
  if (src_col_idx < (K * 4)) {
    *(reinterpret_cast<sycl::uint4*>(qval_reg)) =
        *(reinterpret_cast<const sycl::uint4*>(qdata + src_offset));
  }
  FT scale_reg[4];
  *(reinterpret_cast<sycl::uint2*>(scale_reg)) =
      *(reinterpret_cast<const sycl::uint2*>(scales + params_nidx));
  FT zero_reg[4];
  if (zeros != nullptr) {
    *(reinterpret_cast<sycl::uint2*>(zero_reg)) =
        *(reinterpret_cast<const sycl::uint2*>(zeros + params_nidx));
  }
  FT fval_reg[16];
  ...
  /*
  DPCT1065:369: Consider replacing sycl::nd_item::barrier() with
  sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better
  performance if there is no access to global memory.
  */
  item_ct1.barrier();
  ...
  const int lds_base_offset =
      (item_ct1.get_local_id(2) / 4) * 32 +
      ((item_ct1.get_local_id(2) % 4 + item_ct1.get_local_id(2) / 8) % 4) * 8;
  #pragma unroll
  for (int i = 0; i < 2; ++i) {
    *reinterpret_cast<sycl::uint4*>(fval_reg + i * 8) =
        *reinterpret_cast<sycl::uint4*>(smem + lds_base_offset + i * 32 * 32);
  }

  const auto dst_row_base_kidx =
      item_ct1.get_group(1) * 64 + item_ct1.get_local_id(2) / 4;
  const auto dst_col_nidx =
      item_ct1.get_group(2) * 32 + (item_ct1.get_local_id(2) % 4) * 8;
  #pragma unroll
  for (int i = 0; i < 2; ++i) {
    int dst_row_kidx = dst_row_base_kidx + i * 32;
    int dst_offset = dst_row_kidx * N + dst_col_nidx;
    if (dst_row_kidx < K && dst_col_nidx < N) {
      *reinterpret_cast<sycl::uint4*>(fdata + dst_offset) =
          *reinterpret_cast<sycl::uint4*>(fval_reg + i * 8);
    }
  }
}
...
void restore_N32_K16_dequantize_rhs_w8a16(const QT* qdata, const FT* scales,
  ...
  if (GroupSize == -1) {
    const int BLOCK = 128;
    dpct::dim3 grid(N_32align / 32, ((K / 16) + 3) / 4);
    stream->submit([&](sycl::handler& cgh) {
      sycl::local_accessor<FT, 1> smem_acc_ct1(sycl::range<1>(64 * 32), cgh);
      ...
      cgh.parallel_for(
          ...
          [=](sycl::nd_item<3> item_ct1) {
            restore_N32_K16_dequantize_rhs_w8a16_perc_kernel<FT, QT>(
                qdata, scales, zeros, fdata, N_32align, N, K, item_ct1,
                smem_acc_ct1
...
void w8a16_gemm_dq_cublas(const FT* in, const QT* rhs_qdata_ptr,
                          ...
                          const int N_32align, const int N, const int K,
                          const int group_size, dpct::queue_ptr stream,
                          dpct::blas::descriptor_ptr handle) {
  static_assert(std::is_same<FT, sycl::half>::value ||
                    std::is_same<FT, sycl::ext::oneapi::bfloat16>::value,
  ...
  dpct::library_data_t cuda_type;
  if (std::is_same<FT, sycl::half>::value) {
    cuda_type = dpct::library_data_t::real_half;
  } else {
    cuda_type = dpct::library_data_t::real_bfloat16;
  }
  CHECK_CUBLAS(DPCT_CHECK_ERROR(
      dpct::blas::gemm(handle, oneapi::mkl::transpose::nontrans,
                       oneapi::mkl::transpose::nontrans, N, M, K, &alpha,
                       rhs_fdata_ptr, cuda_type, ldb, in, cuda_type, lda, &beta,
                       out, cuda_type, ldc, dpct::library_data_t::real_float)));
}
...
void allspark_qgemm_w8a16_perc_ampere(
    ...
    dpct::queue_ptr stream, dpct::blas::descriptor_ptr handle) {
...
torch::Tensor allspark_w8a16_gemm(
  ...
  dpct::blas::descriptor_ptr handle = at::cuda::getCurrentCUDABlasHandle();
  ...
  if (a.dtype() == at::ScalarType::Half) {
    allspark::allspark_qgemm_w8a16_perc_ampere<sycl::half, uint8_t>(
        reinterpret_cast<const sycl::half*>(a_ptr), b_ptr,
        reinterpret_cast<const sycl::half*>(b_scale_ptr),
        reinterpret_cast<const sycl::half*>(b_zero_ptr),
        reinterpret_cast<sycl::half*>(c_ptr), m, n_32align, n, k, ws,
        fused_gemm_params, group_size, CUBLAS_M_THRESHOLD, sm_version, stream,
        handle);
  } else if (a.dtype() == at::ScalarType::BFloat16) {
    allspark::allspark_qgemm_w8a16_perc_ampere<sycl::ext::oneapi::bfloat16,
                                               uint8_t>(
        reinterpret_cast<const sycl::ext::oneapi::bfloat16*>(a_ptr), b_ptr,
        reinterpret_cast<const sycl::ext::oneapi::bfloat16*>(b_scale_ptr),
        reinterpret_cast<const sycl::ext::oneapi::bfloat16*>(b_zero_ptr),
        reinterpret_cast<sycl::ext::oneapi::bfloat16*>(c_ptr), m, n_32align, n,
        k, ws, fused_gemm_params, group_size, CUBLAS_M_THRESHOLD, sm_version,
        stream, handle);
  }
