...
/*
DPCT1001:366: The statement could not be removed.
*/
/*
DPCT1000:367: Error handling if-stmt was detected but could not be rewritten.
*/
/*
DPCT1009:368: SYCL reports errors using exceptions and does not use error codes.
Please replace the "get_error_string_dummy(...)" with a real error-handling
function.
*/
#define CHECK_CUDA(cmd)                                                \
  do {                                                                 \
    dpct::err0 cuda_status = cmd;                                      \
    if (cuda_status != 0) {                                            \
      std::string err_str = dpct::get_error_string_dummy(cuda_status); \
...
void f16_gemm_splitk_reduce_kernel(const FType* C_split, FType* C,
                                              uint32_t n, uint32_t n_matrix,
                                              uint32_t matrix_size,
                                              const sycl::nd_item<3> &item_ct1) {
  auto idx = item_ct1.get_group(2) * BLOCK + item_ct1.get_local_id(2);
...
void f16_gemm_splitk_reduce(const FType* C_split, FType* C, const uint32_t m,
  ...
  dpct::kernel_launcher::launch(kernel, grid, BLOCK, 0, stream, C_split, C, n,
                                n_matrix, matrix_size);
}

template <typename T>
struct HalfType;
template <>
struct HalfType<sycl::half> {
  using T1 = sycl::half;
  using T2 = sycl::half2;
};
template <>
struct HalfType<sycl::ext::oneapi::bfloat16> {
  using T1 = sycl::ext::oneapi::bfloat16;
  using T2 = sycl::marray<sycl::ext::oneapi::bfloat16, 2>;
};

// convert 64-bit pointer to 32-bit smem addr
__dpct_inline__ uint32_t smem_u32addr(const void* smem_ptr) {
  uint32_t addr;
  /*
  DPCT1053:181: Migration of device assembly code is not supported.
  */
  asm("{.reg .u64 u64addr;\n"
...
template <typename T>
__dpct_inline__ void ldg16_cg_0(T& r0, const void* ptr, bool guard) {
  static_assert(sizeof(T) == 2, "ldg16_cg_0: invalid T");

  /*
  DPCT1053:182: Migration of device assembly code is not supported.
  */
  asm volatile(
      "{.reg .pred p;\n"
      " setp.ne.b32 p, %2, 0;\n"
      " @!p mov.b16 %0, 0;\n"
#if DPCT_COMPAT_RT_MAJOR_VERSION >= 11 && DPCT_COMPAT_RT_MINOR_VERSION >= 4 && \
...
template <typename T>
__dpct_inline__ void ldg64_ca(T& r0, T& r1, const void* ptr, bool guard) {
...
template <typename T>
__dpct_inline__ void ldg128_cg_0(T& r0, T& r1, T& r2, T& r3, const void* ptr,
                                 bool guard) {
...
template <typename T>
__dpct_inline__ void lds128(T& reg0, T& reg1, T& reg2, T& reg3,
                            const uint32_t addr) {
  static_assert(sizeof(T) == 4, "lds128: invalid T");

  {(reinterpret_cast<uint32_t&>(reg0)), (reinterpret_cast<uint32_t&>(reg1)),
   (reinterpret_cast<uint32_t&>(reg2)), (reinterpret_cast<uint32_t&>(reg3))} =
      *((uint32_t*)(uintptr_t)addr);
}

template <typename T>
__dpct_inline__ void stg128(const T& r0, const T& r1, const T& r2, const T& r3,
...

  {
    ...
    p = (int)guard != 0;
    if (p) {
      *((uint32_t*)(uintptr_t)ptr) = {(reinterpret_cast<const uint32_t&>(r0)),
                                      (reinterpret_cast<const uint32_t&>(r1)),
                                      (reinterpret_cast<const uint32_t&>(r2)),
                                      (reinterpret_cast<const uint32_t&>(r3))};
    }
  }
}

template <typename T>
__dpct_inline__ void ldsm_4(T& r0, T& r1, T& r2, T& r3, const uint32_t& addr) {
  static_assert(sizeof(T) == 4, "ldsm_4: invalid T");
#if (DPCT_COMPATIBILITY_TEMP >= 750) && (DPCT_COMPAT_RT_MAJOR_VERSION >= 11)
  asm volatile(
...
template <typename FType>
__dpct_inline__ void hmma16816_f32(float (&d)[4], const uint32_t (&a)[4],
                                   const uint32_t (&b)[2]);

template <>
__dpct_inline__ void hmma16816_f32<__half>(float (&d)[4],
                                           const uint32_t (&a)[4],
                                           const uint32_t (&b)[2]) {
#if (DPCT_COMPATIBILITY_TEMP >= 800) && (DPCT_COMPAT_RT_MAJOR_VERSION >= 11)
  asm volatile(
...
template <>
__dpct_inline__ void hmma16816_f32<__nv_bfloat16>(float (&d)[4],
                                                  const uint32_t (&a)[4],
                                                  const uint32_t (&b)[2]) {
#if (DPCT_COMPATIBILITY_TEMP >= 800) && (DPCT_COMPAT_RT_MAJOR_VERSION >= 11)
  asm volatile(
...
__dpct_inline__ void cp_async(const uint32_t smem_addr, const void* gmem_ptr,
                              const int src_in_bytes, bool guard) {
  static_assert(
      (SIZE_IN_BYTES == 4 || SIZE_IN_BYTES == 8 || SIZE_IN_BYTES == 16),
      "Size is not supported");
#if DPCT_COMPAT_RT_MAJOR_VERSION >= 11 && DPCT_COMPATIBILITY_TEMP >= 800
  asm volatile(
      "{.reg.pred p;\n"
      " setp.ne.b32 p, %4, 0;\n"
...
template <int SIZE_IN_BYTES>
__dpct_inline__ void cp_async_ca(const uint32_t smem_addr, const void* gmem_ptr,
                                 const int src_in_bytes, bool guard) {
  static_assert(
      (SIZE_IN_BYTES == 4 || SIZE_IN_BYTES == 8 || SIZE_IN_BYTES == 16),
      "Size is not supported");
...
__dpct_inline__ void cp_async_commit_group() {
#if DPCT_COMPAT_RT_MAJOR_VERSION >= 11 && DPCT_COMPATIBILITY_TEMP >= 800
...
template <int N>
__dpct_inline__ void cp_asyc_wait_group() {
#if DPCT_COMPAT_RT_MAJOR_VERSION >= 11 && DPCT_COMPATIBILITY_TEMP >= 800
...
template <typename T>
__dpct_inline__ void cvt_8bx4_to_16bx4_bias128(const uint32_t& idata, T* fdata);

template <>
// fast conversion: 4xuint8 to 4xhalf, subtracting bias = 128
__dpct_inline__ void cvt_8bx4_to_16bx4_bias128<__half2>(const uint32_t& idata,
                                                        sycl::half2* fdata) {
  uint32_t i10, i32;
  /*
  ...
  */
  ...
  fdata[0] = reinterpret_cast<const sycl::half2&>(i10) -
             reinterpret_cast<const sycl::half2&>(MAGIC_NUM);
  fdata[1] = reinterpret_cast<const sycl::half2&>(i32) -
             reinterpret_cast<const sycl::half2&>(MAGIC_NUM);
}
...
__dpct_inline__ void cvt_8bx4_to_16bx4_bias128<__nv_bfloat162>(
    const uint32_t& idata,
    sycl::marray<sycl::ext::oneapi::bfloat16, 2>* fdata) {
  float fp32_imd[4];
  ...
  */
  ...
  /*
  DPCT1053:187: Migration of device assembly code is not supported.
  */
  asm volatile(
...
struct sycl::is_device_copyable<
    allspark::SM8x_GEMM_W8A16_Splitk_Params<FType, QType>>
    : std::true_type {};  // namespace allspark
