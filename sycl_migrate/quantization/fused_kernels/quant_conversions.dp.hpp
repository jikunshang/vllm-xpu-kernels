...
static __dpct_inline__ int8_t float_to_int8_rn(float const x) {
#ifdef USE_ROCM
...
static __dpct_inline__ fp8_type float_to_fp8(float const x) {
  float const r =
...
struct ScaledQuant<
  ...
  static __dpct_inline__ quant_type_t quant_fn(float const x,
                                               float const scale) {
...
void scaled_quant_conversion(quant_type_t* __restrict__ output,
