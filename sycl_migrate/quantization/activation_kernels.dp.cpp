...
template <typename T>
__dpct_inline__ T silu_kernel(const T& x) {
  // x * sigmoid(x)
...
template <typename scalar_t, scalar_t (*ACT_FN)(const scalar_t&),
          typename fp8_type>
void act_and_mul_quant_kernel(
    fp8_type* __restrict__ out,          // [..., d]
    const scalar_t* __restrict__ input,  // [..., 2, d]
    const float* scale, const int d, const sycl::nd_item<3> &item_ct1) {
  const int32_t blocks_per_token = item_ct1.get_group_range(1);
  ...
  const int32_t block_start = item_ct1.get_group(1) * elems_per_block;
  ...
  const sycl::int4* __restrict__ x_128bit_ptr =
      reinterpret_cast<const sycl::int4*>(x_ptr);
  const sycl::int4* __restrict__ y_128bit_ptr =
      reinterpret_cast<const sycl::int4*>(y_ptr);
  sycl::int2* __restrict__ out_128bit_ptr =
      reinterpret_cast<sycl::int2*>(out_ptr);

  float inverted_scale = 1 / *scale;
#pragma unroll
  for (int32_t vec_idx = vec_start_idx + item_ct1.get_local_id(2);
       vec_idx < vec_end_idx; vec_idx += item_ct1.get_local_range(2)) {
    const sycl::int4 x_128bit = VLLM_LDG(&x_128bit_ptr[vec_idx]);
    const sycl::int4 y_128bit = VLLM_LDG(&y_128bit_ptr[vec_idx]);
...
#pragma unroll
    ...
    out_128bit_ptr[vec_idx] = reinterpret_cast<const sycl::int2&>(out_vec);
  }

  // Scalar cleanup code
  if (block_end > vec_loop_end) {
    for (int64_t idx = vec_loop_end + item_ct1.get_local_id(2); idx < block_end;
...
#define LAUNCH_ACTIVATION_GATE_KERNEL(KERNEL)                                 \
  ...
  dpct::dim3 grid(num_tokens, num_tokens > 16 ? num_tokens > 32 ? 1 : 2 : 4); \
  dpct::dim3 block(std::min(d, 512));                                         \
