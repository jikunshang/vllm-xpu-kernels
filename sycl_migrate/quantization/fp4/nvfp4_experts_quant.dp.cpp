...
template <>
struct TypeConverter<sycl::half2> {
...
template <>
struct TypeConverter<sycl::marray<sycl::ext::oneapi::bfloat16, 2>> {
...
inline uint32_t fp32_vec_to_e2m1(float (&array)[8]) {
#if defined(DPCT_COMPATIBILITY_TEMP) && (DPCT_COMPATIBILITY_TEMP >= 1000)
  uint32_t val;
  asm volatile(
      "{\n"
      ".reg .b8 byte0;\n"
...
inline uint32_t fp32_vec_to_e2m1(sycl::float2 (&array)[4]) {
#if defined(DPCT_COMPATIBILITY_TEMP) && (DPCT_COMPATIBILITY_TEMP >= 1000)
  uint32_t val;
  asm volatile(
      "{\n"
...
// Fast reciprocal.
inline float reciprocal_approximate_ftz(float a) {
  float b;
  b = 1 / a;
  return b;
}
...
uint8_t* cvt_quant_to_fp4_get_sf_out_offset(int rowIdx, int colIdx,
                                                       int numCols,
                                                       SFType* SFout) {
#if defined(DPCT_COMPATIBILITY_TEMP) && (DPCT_COMPATIBILITY_TEMP >= 1000)
...
uint32_t cvt_warp_fp16_to_fp4(PackedVec<Type>& vec, float SFScaleVal,
                                         uint8_t* SFout) {
#if defined(DPCT_COMPATIBILITY_TEMP) && (DPCT_COMPATIBILITY_TEMP >= 1000)
...
void quant_impl(void* output, void* output_scale, void* input,
                ...
                int n_experts, dpct::queue_ptr stream) {
  // TODO: this multiProcessorCount should be cached.
  int device;
  device = dpct::get_current_device_id();
  int multiProcessorCount;
  multiProcessorCount = dpct::get_device(device).get_max_compute_units();

  // Grid, Block size.
  // Each thread converts 8 values.
  int const workSizePerRow = k / ELTS_PER_THREAD;
  int const totalWorkSize = m_topk * workSizePerRow;
  dpct::dim3 block(std::min(workSizePerRow, 512));
  // Get number of blocks per SM (assume we can fully utilize the SM).
  int const numBlocksPerSM = 2048 / block.x;
  dpct::dim3 grid(
      std::min(static_cast<int>((totalWorkSize + block.x - 1) / block.x),
  ...
  if (blockRepeat > 1) {
    /*
    DPCT1083:223: The size of local memory in the migrated code may be different
    ...
    */
    ...
    if (n_experts >= 16) {
      ...
      */
      ...
      /*
      DPCT1049:226: The work-group size passed to the SYCL kernel may exceed the
      limit. To get the device limit, query info::device::max_work_group_size.
      Adjust the work-group size if needed.
      */
      stream->parallel_for(
