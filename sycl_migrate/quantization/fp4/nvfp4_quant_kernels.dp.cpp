...
void invokeFP4Quantization(int m, int n, T const* input, float const* SFScale,
                           ...
                           int multiProcessorCount, dpct::queue_ptr stream) {
  // Grid, Block size.
  // Each thread converts 8 values.
  dpct::dim3 block(std::min(int(n / ELTS_PER_THREAD), 512));
  // Get number of blocks per SM (assume we can fully utilize the SM).
  int const numBlocksPerSM = 2048 / block.x;
  dpct::dim3 grid(std::min(int(m), multiProcessorCount * numBlocksPerSM));
  ...
  if (useUE8M0) {
    ...
    */
    stream->parallel_for(
        sycl::nd_range<3>(grid * block, block), [=](sycl::nd_item<3> item_ct1) {
          cvt_fp16_to_fp4<T, true>(m, n, input, SFScale,
    ...
    */
    stream->parallel_for(
        sycl::nd_range<3>(grid * block, block), [=](sycl::nd_item<3> item_ct1) {
          cvt_fp16_to_fp4<T, false>(m, n, input, SFScale,
                                    reinterpret_cast<uint32_t*>(output),
                                    reinterpret_cast<uint32_t*>(SFOuput));
        });
  }
...
void scaled_fp4_quant_sm100a(torch::Tensor const& output,
  ...
  int multiProcessorCount = get_device_attribute(16, -1);
  ...
  switch (input.scalar_type()) {
    case torch::kHalf: {
      auto input_ptr = reinterpret_cast<sycl::half const*>(input.data_ptr());
      invokeFP4Quantization(m, n, input_ptr, input_sf_ptr, output_ptr, sf_out,
                            useUE8M0, multiProcessorCount, stream);
      break;
    }
    case torch::kBFloat16: {
      auto input_ptr = reinterpret_cast<sycl::ext::oneapi::bfloat16 const*>(
          input.data_ptr());
      invokeFP4Quantization(m, n, input_ptr, input_sf_ptr, output_ptr, sf_out,
                            useUE8M0, multiProcessorCount, stream);
      break;
