...
using FragS_GROUP =
    Vec<sycl::half2, 1>;  // weight per-group quantization scales
using FragS_CHANNEL =
...
inline void cp_async1(void* smem_ptr, const void* glob_ptr) {
  const int BYTES = 4;
  auto smem = smem_ptr;
  /*
...
// output/accumulation.
inline void mma(const FragA& a_frag, const FragB& frag_b,
                           FragC& frag_c) {
...
inline sycl::half2 float2_to_half2(sycl::float2 f) {
  ...
  uint16_t h0, h1;
  h0 = sycl::vec<float, 1>(f.x())
           .template convert<sycl::half, sycl::rounding_mode::rte>()
           .template as<sycl::vec<uint16_t, 1>>()
           .x();
  h1 = sycl::vec<float, 1>(f.y())
           .template convert<sycl::half, sycl::rounding_mode::rte>()
           .template as<sycl::vec<uint16_t, 1>>()
           .x();
  res =
      sycl::vec<uint16_t, 2>({h0, h1}).template as<sycl::vec<uint32_t, 1>>()[0];
  return reinterpret_cast<sycl::half2&>(res);
}

inline float int32_to_float(int h) {
  float res;
  res = sycl::vec<int32_t, 1>(h)
...
inline FragB dequant_per_channel(int q) {
...
inline FragB dequant_per_group(int q, FragS_GROUP& frag_s, int i) {
  ...
  *reinterpret_cast<sycl::half2*>(&t0) =
      *reinterpret_cast<sycl::half2*>(&t0) -
      *reinterpret_cast<const sycl::half2*>(&SUB);
  *reinterpret_cast<sycl::half2*>(&t1) =
      sycl::fma(*reinterpret_cast<sycl::half2*>(&t1),
  ...
  double_s =
      sycl::vec<uint16_t, 2>({s, s}).template as<sycl::vec<uint32_t, 1>>()[0];
  // dequant and convert 4 half to 4 uint8 (be placed at the low 8 bits of 4
  // half, respectively)
  static constexpr uint32_t MAGIC_NUM = 0x64806480;
  *reinterpret_cast<sycl::half2*>(&t0) =
      sycl::fma(*reinterpret_cast<sycl::half2*>(&t0),
                *reinterpret_cast<sycl::half2*>(&double_s),
                *reinterpret_cast<const sycl::half2*>(&MAGIC_NUM));
  *reinterpret_cast<sycl::half2*>(&t1) =
      sycl::fma(*reinterpret_cast<sycl::half2*>(&t1),
                *reinterpret_cast<sycl::half2*>(&double_s),
                *reinterpret_cast<const sycl::half2*>(&MAGIC_NUM));
...
template <const int threads,          // number of threads in a threadblock
          ...
          >
/*
DPCT1110:63: The total declared local variable size in device function Marlin
...
*/
void Marlin(
    const sycl::int4* __restrict__ A,  // int8 input matrix of shape mxk
    const sycl::int4* __restrict__ B,  // 4bit quantized weight matrix of shape
                                       // kxn
    sycl::int4* __restrict__ C,        // int32 global_reduce buffer of shape
                                 // (max_par*16*4)xn, as int8 tensor core's
                                 // output is int32 dtype
    sycl::int4* __restrict__ D,       // fp16 output buffer of shape mxn
    const float* __restrict__ s_tok,  // fp32 activation per-token quantization
                                      // scales of shape mx1
    const sycl::int4* __restrict__ s_ch,  // fp32 weight per-channel
                                          // quantization scales of shape 1xn
    const sycl::
        int4* __restrict__ s_group,  // fp16 weight per-group quantization
                                     // scales of shape (k/groupsize)xn, when
                                     // group_blocks=-1, it should be nullptr
    int prob_m,                      // batch dimension m
    int prob_n,                      // output dimension n
    int prob_k,                      // reduction dimension k
    int* locks, const sycl::nd_item<3>& item_ct1,
  ...
  int iters =
      ceildiv(k_tiles * n_tiles * parallel, item_ct1.get_group_range(2));
  ...
  // synchronization.
  auto init_slice = [&](const sycl::nd_item<3> &item_ct1) {
    slice_iters = iters * (item_ct1.get_group(2) + 1) -
                  (k_tiles * slice_col_par + slice_row);
  ...
  int a_gl_rd = a_gl_stride * (item_ct1.get_local_id(2) / a_gl_rd_delta_o) +
                (item_ct1.get_local_id(2) % a_gl_rd_delta_o);
  a_gl_rd += a_gl_rd_delta_o * slice_row;
  // Shared write index of current thread.
  int a_sh_wr = a_sh_stride * (item_ct1.get_local_id(2) / a_gl_rd_delta_o) +
                (item_ct1.get_local_id(2) % a_gl_rd_delta_o);
  // Shared read index.
  // NOTE(HandH1998): int8 input a only need 16 threads to load 16x16 matrix
  int a_sh_rd = a_sh_stride * ((item_ct1.get_local_id(2) % 32) % 16);
  a_sh_rd += 1 * ((item_ct1.get_local_id(2) / 32) / (thread_n_blocks / 4));

  int b_gl_rd = b_gl_stride * (item_ct1.get_local_id(2) / b_sh_stride) +
                (item_ct1.get_local_id(2) % b_sh_stride);
  b_gl_rd += b_sh_stride * slice_col;
  b_gl_rd += b_gl_rd_delta_o * slice_row;
  auto b_sh_wr = item_ct1.get_local_id(2);
  auto b_sh_rd = item_ct1.get_local_id(2);

  auto s_tok_gl_rd = item_ct1.get_local_id(2);
  ...
  int s_tok_sh_wr = (item_ct1.get_local_id(2) / 16) * 16 +
                    (item_ct1.get_local_id(2) % 8) * 2 +
                    (item_ct1.get_local_id(2) % 16) / 8;
  int s_tok_sh_rd = (item_ct1.get_local_id(2) % 32) / 4;
  bool s_tok_sh_wr_pred = item_ct1.get_local_id(2) < prob_m;

  auto s_ch_gl_rd = s_ch_sh_stride * slice_col + item_ct1.get_local_id(2);
  auto s_ch_sh_wr = item_ct1.get_local_id(2);
  int s_ch_sh_rd =
      16 * ((item_ct1.get_local_id(2) / 32) % (thread_n_blocks / 4)) +
      2 * ((item_ct1.get_local_id(2) % 32) % 4);
  bool s_ch_sh_wr_pred = item_ct1.get_local_id(2) < s_ch_sh_stride;
  ...
  if constexpr (group_blocks != -1) {
    s_group_gl_rd =
        ...
        s_group_sh_stride * slice_col + item_ct1.get_local_id(2);
    s_group_sh_wr = item_ct1.get_local_id(2);
    // NOTE(HandH1998): s_group_sh_rd is related to mma output C
    s_group_sh_rd =
        8 * ((item_ct1.get_local_id(2) / 32) % (thread_n_blocks / 4)) +
        (item_ct1.get_local_id(2) % 32) / 4;
    s_group_sh_wr_pred = item_ct1.get_local_id(2) < s_group_sh_stride;
  }
  ...
  sycl::int4* sh_s_tok = sh_b + (stages * b_sh_stage);
  sycl::int4* sh_s_ch = sh_s_tok + s_tok_sh_stride;
  sycl::int4* sh_s_group = sh_s_ch + s_ch_sh_stride;
  ...
  // Zero accumulators.
  auto zero_accums = [&](const sycl::nd_item<3> &item_ct1) {
  #pragma unroll
  ...
  #pragma unroll
      ...
      if constexpr (group_blocks != -1) {
        if (pipe % (group_blocks / thread_k_blocks) == 0) {
          sycl::int4* sh_s_group_stage = sh_s_group + s_group_sh_stage * pipe;
  ...
  auto wait_for_stage = [&](const sycl::nd_item<3> &item_ct1) {
  ...
  auto fetch_to_registers = [&](int k, int pipe) {
    ...
    if constexpr (group_blocks != -1) {
      sycl::int4* sh_s_group_stage =
          sh_s_group +
          s_group_sh_stage * ((group_blocks / thread_k_blocks) *
                              (pipe / (group_blocks / thread_k_blocks)));
      reinterpret_cast<sycl::int4*>(&frag_s_group[k % 2])[0] =
  ...
  auto thread_block_reduce = [&](const sycl::nd_item<3> &item_ct1) {
    constexpr int red_off = threads / b_sh_stride / 2;
    if (red_off >= 1) {
      auto red_idx = item_ct1.get_local_id(2) / b_sh_stride;
      constexpr int red_sh_stride = b_sh_stride * 4 * 2;
      constexpr int red_sh_delta = b_sh_stride;
      int red_sh_rd = red_sh_stride * (item_ct1.get_local_id(2) / b_sh_stride) +
                      (item_ct1.get_local_id(2) % b_sh_stride);
  ...
  #pragma unroll
              ...
              sh[red_sh_wr] =
                  reinterpret_cast<sycl::int4*>(&frag_c)[4 * 2 * m_block + j];
            }
          }
          /*
  ...
  auto global_reduce = [&](const sycl::nd_item<3> &item_ct1, bool first = false, bool last = false) {
    ...
    if (item_ct1.get_local_id(2) < active_threads) {
      ...
      int c_gl_wr = c_gl_stride * ((item_ct1.get_local_id(2) % 32) / 4) +
                    8 * (item_ct1.get_local_id(2) / 32) +
                    (item_ct1.get_local_id(2) % 4) * 2;
      c_gl_wr += (4 * thread_n_blocks) * slice_col;
      constexpr int c_sh_wr_delta = active_threads * 2;
      auto c_sh_wr = 2 * item_ct1.get_local_id(2);
  ...
  #pragma unroll
      for (int i = 0; i < thread_m_blocks * 4; i++) {
        if (i < (thread_m_blocks - 1) * 4 || 8 * (i / 2) + row < prob_m) {
          if (!first) {
            sycl::int4 d_red1 = sh[c_sh_wr + i * c_sh_wr_delta];
            sycl::int4 d_red2 = sh[c_sh_wr + i * c_sh_wr_delta + 1];
  #pragma unroll
            for (int j = 0; j < 4; j++) {
  ...
  // in fragment layout.
  auto write_result = [&](const sycl::nd_item<3> &item_ct1) {
    ...
    int d_gl_wr =
        d_gl_stride * (item_ct1.get_local_id(2) / (2 * thread_n_blocks)) +
        (item_ct1.get_local_id(2) % (2 * thread_n_blocks));
    d_gl_wr += (2 * thread_n_blocks) * slice_col;
    int d_sh_wr = (4 * d_sh_stride) * ((item_ct1.get_local_id(2) % 32) / 4) +
                  (item_ct1.get_local_id(2) % 32) % 4;
    d_sh_wr += 32 * (item_ct1.get_local_id(2) / 32);
    int d_sh_rd =
        d_sh_stride * (item_ct1.get_local_id(2) / (2 * thread_n_blocks)) +
        (item_ct1.get_local_id(2) % (2 * thread_n_blocks));
    ...
    auto write = [&](int idx, int c0, int c1, float a_s, FragS_CHANNEL& w_s) {
      ...
      ((sycl::half2*)sh)[idx] = float2_to_half2(deq_res);
    };
  ...
  auto start_pipes = [&](const sycl::nd_item<3> &item_ct1) {
  #pragma unroll
  ...
  #pragma unroll
          ...
          reinterpret_cast<sycl::int4*>(&frag_s_ch)[0] =
              sh_s_ch[s_ch_sh_rd + 0];
          reinterpret_cast<sycl::int4*>(&frag_s_ch)[1] =
              sh_s_ch[s_ch_sh_rd + 1];
          reinterpret_cast<sycl::int4*>(&frag_s_ch)[2] =
              sh_s_ch[s_ch_sh_rd + 8];
          reinterpret_cast<sycl::int4*>(&frag_s_ch)[3] =
              sh_s_ch[s_ch_sh_rd + 9];
        }
      }
      if (slice_count > 1) {  // only globally reduce if there is more than one
                              // block in a slice
        barrier_acquire(&locks[slice_col], slice_idx, item_ct1);
        global_reduce(slice_idx == 0, last);
        barrier_release(&locks[slice_col], item_ct1, last);
      }
      ...
      init_slice();
      if (slice_iters) {
        a_gl_rd = a_gl_stride * (item_ct1.get_local_id(2) / a_gl_rd_delta_o) +
                  (item_ct1.get_local_id(2) % a_gl_rd_delta_o);
  #pragma unroll
        for (int i = 0; i < b_sh_wr_iters; i++)
  ...
  #pragma unroll
        ...
        s_group_gl_rd =
            s_group_sh_stride * slice_col + item_ct1.get_local_id(2);
        s_ch_gl_rd = s_ch_sh_stride * slice_col + item_ct1.get_local_id(2);
        start_pipes();
      }
    }
  }
...
typedef struct dpct_type_113518 {
  int thread_k;
  int thread_n;
  int num_threads;
} thread_config_t;
...
void marlin_qqq_cuda(const void* A, const void* B, void* C, void* D,
  ...
  /*
  DPCT1019:336: local_mem_size in SYCL is not a complete equivalent of
  ...
  */
  ...
  sycl::int4* D_ptr = (sycl::int4*)D;
  const float* s_tok_ptr = (const float*)s_tok;
  const sycl::int4* s_ch_ptr = (const sycl::int4*)s_ch;
  const sycl::int4* s_group_ptr = (const sycl::int4*)s_group;
