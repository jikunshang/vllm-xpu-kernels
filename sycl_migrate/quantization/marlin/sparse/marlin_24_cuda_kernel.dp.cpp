...
template <const int num_bits,         // weight bits
          ...
          >
/*
DPCT1110:48: The total declared local variable size in device function Marlin_24
...
void Marlin_24(
    ...
    const sycl::int4* __restrict__ B,  // 4bit quantized weight matrix of shape
                                       // kxn
    const sycl::int4* __restrict__ meta,  // 2bit metadata information about 2:4
                                          // format on B
    sycl::int4* __restrict__ C,           // fp16 output buffer of shape mxn
    const sycl::int4* __restrict__ s,     // fp16 quantization scales of shape
                                          // (k/groupsize)xn
    int prob_m,                           // batch dimension m
    int prob_n,                           // output dimension n
    int prob_k,                           // reduction dimension k
  ...
  int a_sh_rd = a_sh_stride * ((item_ct1.get_local_id(2) % 32) % 16) +
                (item_ct1.get_local_id(2) % 32) / 16;
  a_sh_rd += 4 * ((item_ct1.get_local_id(2) / 32) / (thread_n_blocks / 4));

  int b_gl_rd =
      b_gl_stride * (item_ct1.get_local_id(2) / b_sh_stride_threads) +
      (item_ct1.get_local_id(2) % b_sh_stride_threads) * b_thread_vecs;
  b_gl_rd += b_sh_stride * slice_col;
  b_gl_rd += b_gl_rd_delta_o * slice_row;
  auto b_sh_wr = item_ct1.get_local_id(2) * b_thread_vecs;
  auto b_sh_rd = item_ct1.get_local_id(2) * b_thread_vecs;

  int m_gl_rd = m_gl_stride * (item_ct1.get_local_id(2) / (m_sh_stride)) +
                (item_ct1.get_local_id(2) % (m_sh_stride));
  m_gl_rd += (m_sh_stride)*slice_col;
  m_gl_rd += m_gl_rd_delta_o * slice_row;
  auto m_sh_wr = item_ct1.get_local_id(2);
  auto m_sh_rd =
      item_ct1.get_local_id(2) % 16 + (item_ct1.get_local_id(2) / 32) * 16;
  ...
  if constexpr (group_blocks == -1) {
    ...
    s_gl_rd = s_gl_stride * ((thread_k_blocks * slice_row) / group_blocks) +
              s_sh_stride * slice_col + item_ct1.get_local_id(2);
  }

  auto s_sh_wr = item_ct1.get_local_id(2);
  int s_sh_rd;
  // We use a different scale layout for grouped and column-wise quantization as
  // we scale a `half2` tile in column-major layout in the former and in
  // row-major in the latter case.
  s_sh_rd = 8 * ((item_ct1.get_local_id(2) / 32) % (thread_n_blocks / 4)) +
            (item_ct1.get_local_id(2) % 32) /
                4;  // Note that in the original Marlin kernel
  ...
  bool m_sh_wr_pred = item_ct1.get_local_id(2) < m_sh_wr_delta;
  const sycl::int4* meta_ptr[m_sh_iters];
  #pragma unroll
  ...
  sycl::int4* sh_m = sh_s + (stages * s_sh_stage);
  ...
  #pragma unroll
      ...
      sycl::int4* sh_meta_stage = sh_m + m_sh_stage * pipe;
  #pragma unroll
      for (int i = 0; i < m_sh_iters; i++) {
        if (m_sh_wr_pred)
  ...
  #pragma unroll
    ...
    sycl::int4* sh_m_stage = sh_m + m_sh_stage * pipe;
  ...
  auto thread_block_reduce = [&](const sycl::nd_item<3> &item_ct1) {
    ...
    if (red_off >= 1) {
      auto red_idx = item_ct1.get_local_id(2) / b_sh_stride_threads;
      constexpr int red_sh_stride = b_sh_stride_threads * 4 * 2;
      constexpr int red_sh_delta = b_sh_stride_threads;
      int red_sh_rd =
          red_sh_stride * (item_ct1.get_local_id(2) / b_sh_stride_threads) +
  ...
  auto global_reduce = [&](const sycl::nd_item<3> &item_ct1, bool first = false, bool last = false) {
    ...
    if (item_ct1.get_local_id(2) < active_threads) {
      ...
      int c_gl_wr = 2 * c_gl_stride * (item_ct1.get_local_id(2) % 4) +
                    8 * (item_ct1.get_local_id(2) / 32) +
                    (item_ct1.get_local_id(2) % 32) / 4;
      c_gl_wr += (2 * thread_n_blocks) * slice_col;
      constexpr int c_sh_wr_delta = active_threads;
      auto c_sh_wr = item_ct1.get_local_id(2);

      int col = 2 * ((item_ct1.get_local_id(2) % 32) % 4);
              ...
              for (int j1 = 0; j1 < 4; j1++) {
                reinterpret_cast<float*>(
                    ...
                    sycl::vec<sycl::half, 1>(
                        reinterpret_cast<sycl::half*>(&c_red)[(j2 * 4 + j1)])
  ...
  #pragma unroll
              for (int j1 = 0; j1 < 4; j1++) {
                reinterpret_cast<sycl::half*>(&c)[(j2 * 4 + j1)] =
                    sycl::vec<float, 1>(
                        reinterpret_cast<float*>(
                            &frag_c)[4 * 2 * 4 * (i / 4) + 8 * j1 + 2 * j2 +
                                     4 * ((i % 4) / 2) + i % 2])
  ...
  auto write_result = [&](const sycl::nd_item<3> &item_ct1) {
    ...
    int c_gl_wr =
        c_gl_stride * (item_ct1.get_local_id(2) / (2 * thread_n_blocks)) +
        (item_ct1.get_local_id(2) % (2 * thread_n_blocks));
    c_gl_wr += (2 * thread_n_blocks) * slice_col;

    int c_sh_wr = c_sh_stride_2 * ((item_ct1.get_local_id(2) % 32) % 4) +
                  ((item_ct1.get_local_id(2) % 32) / 4);  // RLC:
    c_sh_wr += 8 * (item_ct1.get_local_id(2) / 32);       // 128/4(half4)

    constexpr int c_sh_rd_delta =
        c_sh_stride_3 * (threads / (2 * 2 * thread_n_blocks));  // RLC:
    int c_sh_rd =
        c_sh_stride_3 * (item_ct1.get_local_id(2) / (2 * 2 * thread_n_blocks)) +
        (item_ct1.get_local_id(2) % (2 * 2 * thread_n_blocks));
    ...
    auto write = [&](int idx, float c0, float c1, float c2, float c3, FragS& s0,
      ...
      ((sycl::int4*)sh)[idx] = *((sycl::int4*)&res[0]);
    };
    ...
    if (slice_iters == 0) {
      ...
      if constexpr (group_blocks == -1) {
        if constexpr (num_bits == 8) {
          cp_async_wait<0>();
          /*
          DPCT1118:49: SYCL group functions and algorithms must be encountered
          ...
          */
          item_ct1.barrier();
          if (item_ct1.get_local_id(2) / 32 < thread_n_blocks / 4) {
            *(sycl::float4*)(frag_s) = *(sycl::float4*)(&sh_s[s_sh_rd]);
          }
        } else {
          if (last) {
            cp_async_wait<0>();
            /*
            DPCT1118:50: SYCL group functions and algorithms must be encountered
            in converged control flow. You may need to adjust the code.
            */
            /*
...
void marlin_cuda_2_4(const void* A, const void* B, const void* meta, void* C,
  ...
  /*
  DPCT1019:323: local_mem_size in SYCL is not a complete equivalent of
  ...
  */
  ...
  const sycl::int4* meta_ptr = (const sycl::int4*)meta;
