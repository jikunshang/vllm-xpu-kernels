...
template <const int threads,          // number of threads in a threadblock
          ...
          >
/*
DPCT1110:57: The total declared local variable size in device function Marlin
...
void Marlin(
  ...
  #pragma unroll
        ...
        /*
        DPCT1065:327: Consider replacing sycl::nd_item::barrier() with
        sycl::nd_item::barrier(sycl::access::fence_space::local_space) for
        better performance if there is no access to global memory.
        */
        item_ct1.barrier();
            ...
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<float*>(
                  ...
                  sycl::vec<sycl::half, 1>(
                      reinterpret_cast<sycl::half*>(&c_red)[j])
  ...
  #pragma unroll
            for (int j = 0; j < 2 * 4; j++) {
              reinterpret_cast<sycl::half*>(&c)[j] =
                  sycl::vec<float, 1>(
                      reinterpret_cast<float*>(
                          &frag_c)[4 * 2 * 4 * (i / 4) + 4 * j + (i % 4)])
  ...
  auto write_result = [&](const sycl::nd_item<3> &item_ct1) {
    ...
    c_gl_wr += (2 * thread_n_blocks) * slice_col;
    int c_sh_wr = (4 * c_sh_stride) * ((item_ct1.get_local_id(2) % 32) / 4) +
                  (item_ct1.get_local_id(2) % 32) % 4;
    c_sh_wr += 32 * (item_ct1.get_local_id(2) / 32);
    int c_sh_rd =
        c_sh_stride * (item_ct1.get_local_id(2) / (2 * thread_n_blocks)) +
        (item_ct1.get_local_id(2) % (2 * thread_n_blocks));
...
typedef struct dpct_type_612428 {
  int thread_k;
  int thread_n;
  int num_threads;
} thread_config_t;
