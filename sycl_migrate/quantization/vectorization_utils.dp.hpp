...
struct DefaultVecOp {
  ...
  __dpct_inline__ void operator()(vec_n_t<OutT, VEC_SIZE>& dst,
...
template <int VEC_SIZE, typename InT, typename OutT, typename ScaOp>
__dpct_inline__ void vectorize_with_alignment(const InT* in, OutT* out, int len,
                                              int tid, int stride,
                                              ScaOp&& scalar_op) {
...
struct DefaultReadVecOp {
  ...
  __dpct_inline__ void operator()(const vec_n_t<InT, VEC_SIZE>& src) const {
#pragma unroll
    for (int i = 0; i < VEC_SIZE; ++i) {
      scalar_op(src.val[i]);
    }
...
inline void vectorize_read_with_alignment(const InT* in, int len,
  ...
  prefix_elems /= sizeof(InT);
  prefix_elems = sycl::min(prefix_elems, len);
...
template <int VEC_SIZE, typename InT, typename ScaOp>
__dpct_inline__ void vectorize_read_with_alignment(const InT* in, int len,
                                                   int tid, int stride,
                                                   ScaOp&& scalar_op) {
