...
void gptq_marlin_repack_kernel(
    ...
    int size_k, int size_n, const sycl::nd_item<3> &item_ct1,
    uint8_t *dpct_local) {
  constexpr int pack_factor = 32 / num_bits;

  int k_tiles = size_k / tile_k_size;
  int n_tiles = size_n / tile_n_size;
  int block_k_tiles = div_ceil(k_tiles, item_ct1.get_group_range(2));

  auto start_k_tile = item_ct1.get_group(2) * block_k_tiles;
  if (start_k_tile >= k_tiles) {
    return;
  }

  int finish_k_tile = dpct::min(start_k_tile + block_k_tiles, k_tiles);
  ...
  sycl::int4* sh_perm_ptr = sh;
  sycl::int4* sh_pipe_ptr = sh_perm_ptr;
  ...
  auto load_perm_to_shared = [&](int k_tile_id,
    ...
    sycl::int4 const* perm_int4_ptr =
        reinterpret_cast<sycl::int4 const*>(perm_ptr);

    if (item_ct1.get_local_id(2) < perm_size) {
      sh_perm_ptr[item_ct1.get_local_id(2)] =
          perm_int4_ptr[first_k_int4 + item_ct1.get_local_id(2)];
    }
    /*
  ...
  auto fetch_to_shared = [&](int pipe, int k_tile_id, int n_tile_id,
    ...
    sycl::int4* sh_ptr = sh_pipe_ptr + stage_size * pipe;

    if constexpr (has_perm) {
      if (item_ct1.get_local_id(2) < stage_size) {
        auto k_id = item_ct1.get_local_id(2) / stage_n_threads;
        auto n_id = item_ct1.get_local_id(2) % stage_n_threads;
        ...
        cp_async4(
            ...
            reinterpret_cast<sycl::int4 const*>(&(
  ...
  operator() exceeds 128 bytes and may cause high register pressure. Consult
  ...
  */
  auto repack_tile = [&](int pipe, int k_tile_id, int n_tile_id,
    ...
    auto th_id = item_ct1.get_local_id(2) % 32;
    ...
    sycl::int4* sh_stage_ptr = sh_pipe_ptr + stage_size * pipe;
...
torch::Tensor gptq_marlin_repack(torch::Tensor& b_q_weight, torch::Tensor& perm,
  ...
  /*
  DPCT1019:354: local_mem_size in SYCL is not a complete equivalent of
