...
void MarlinDefault(MARLIN_KERNEL_PARAMS) {}

// Auto generated SYCL kernel wrapper used to migration kernel function pointer.
void MarlinDefault_wrapper(
    const sycl::int4* __restrict A, const sycl::int4* __restrict B,
    sycl::int4* __restrict C, sycl::int4* __restrict C_tmp,
    const sycl::int4* __restrict scales_ptr,
    const uint16_t* __restrict scale2_ptr, const sycl::int4* __restrict zp_ptr,
    const int* __restrict g_idx, int num_groups, int prob_m, int prob_n,
    int prob_k, int lda, int* locks, bool use_atomic_add, bool use_fp32_reduce,
    int max_shared_mem) {
  ...
  queue.parallel_for(nr, [=](sycl::nd_item<3> item_ct1) {
    MarlinDefault(A, B, C, C_tmp, scales_ptr, scale2_ptr, zp_ptr, g_idx,
                  num_groups, prob_m, prob_n, prob_k, lda, locks,
                  use_atomic_add, use_fp32_reduce, max_shared_mem);
  });
};
...
void permute_cols_kernel(sycl::int4 const* __restrict__ a_int4_ptr,
  ...
  auto start_row = block_rows * item_ct1.get_group(2);
  ...
  int input_row_stride = lda * sizeof(sycl::half) / 16;
  int output_row_stride = size_k * sizeof(sycl::half) / 16;
  ...
  auto permute_row = [&](int row, const sycl::nd_item<3> &item_ct1) {
    ...
    sycl::half const* a_row_half =
        reinterpret_cast<sycl::half const*>(a_int4_ptr + input_offset);
    sycl::half* out_half =
        reinterpret_cast<sycl::half*>(out_int4_ptr + output_offset);

    int base_k = 0;

    for (int i = 0; i < iters; i++) {
      auto cur_k = base_k + item_ct1.get_local_id(2);
...
typedef struct dpct_type_149684 {
  int thread_k;
  int thread_n;
  int num_threads;
} thread_config_t;
...
typedef struct dpct_type_166637 {
  int blocks_per_sm;
...
MarlinFuncPtr get_marlin_kernel(const vllm::ScalarType q_type,
  ...
  auto kernel = MarlinDefault_wrapper;
  if (false) {
  }
  ...
  if (std::is_same<scalar_t, sycl::half>::value) {
    if (false) {
    }
    FZP_GET_IF(vllm::kU4)
  }
...
exec_config_t determine_exec_config(const vllm::ScalarType& q_type, int prob_m,
  ...
  for (int i = 0; i < thread_configs_size; i++) {
    ...
    if (kernel == MarlinDefault_wrapper) continue;
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* s,
               ...
               int dev, dpct::queue_ptr stream, int thread_k_init,
  ...
  sycl::int4* C_tmp_ptr = (sycl::int4*)C_tmp;
  const sycl::int4* s_ptr = (const sycl::int4*)s;
  const uint16_t* s2_ptr = (const uint16_t*)s2;
  const sycl::int4* zp_ptr = (const sycl::int4*)zp;
  const int* g_idx_ptr = (const int*)g_idx;
  const int* perm_ptr = (const int*)perm;
  sycl::int4* a_tmp_ptr = (sycl::int4*)a_tmp;
  ...
  if (has_act_order) {
    ...
    // clang-format off
    {
      ...
      stream->parallel_for(
        sycl::nd_range<3>(sycl::range<3>(1, 1, sms) * sycl::range<3>(1, 1, default_threads), sycl::range<3>(1, 1, default_threads)), 
        [=](sycl::nd_item<3> item_ct1) {
          permute_cols_kernel(A_ptr, perm_ptr, a_tmp_ptr, prob_m, prob_k, lda, block_rows, item_ct1);
        });
    }
    // clang-format on
    A_ptr = a_tmp_ptr;
  ...
  /*
  DPCT1019:355: local_mem_size in SYCL is not a complete equivalent of
  ...
  */
  ...
  while (rest_m) {
    ...
    if (kernel == MarlinDefault_wrapper) {
    ...
    // avoid ">>>" being formatted to "> > >"
    // clang-format off
    dpct::kernel_launcher::launch(kernel, blocks, num_threads, max_shared_mem_new, stream, A_ptr, B_ptr, C_ptr, C_tmp_ptr, s_ptr, s2_ptr, zp_ptr, g_idx_ptr, num_groups, prob_m_split, prob_n, prob_k, lda, locks, part_use_atomic_add, use_fp32_reduce, max_shared_mem_new);
    // clang-format on
