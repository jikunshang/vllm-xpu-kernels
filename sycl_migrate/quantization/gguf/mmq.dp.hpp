...
/*
DPCT1110:96: The total declared local variable size in device function mul_mat_q
...
#pragma unroll
            for (int i = 0; i < mmq_x; i += nwarps) {
                ...
                const int index_y =
                    (item_ct1.get_local_id(1) + i) * WARP_SIZE_GGUF +
                    kqs % WARP_SIZE_GGUF;
...
#pragma unroll
            for (int ids0 = 0; ids0 < mmq_x; ids0 += nwarps * QI8_1) {
                const int ids =
                    (ids0 + item_ct1.get_local_id(1) * QI8_1 +
                     item_ct1.get_local_id(2) / (WARP_SIZE_GGUF / QI8_1)) %
                    mmq_x;
                ...
                const sycl::half2* dsi_src =
                    &y[col_y_eff * blocks_per_col_y + ib0 * (qk / QK8_1) +
                       ir * (WARP_SIZE_GGUF / QI8_1) + kby]
                         .ds;
                sycl::half2* dsi_dst = &tile_y_ds[ids * (WARP_SIZE_GGUF / QI8_1) + kby];
                if (need_sum) {
                    *dsi_dst = *dsi_src;
                } else {
            ...
            /*
            DPCT1118:97: SYCL group functions and algorithms must be encountered
...
#pragma unroll
            ...
            /*
            DPCT1118:98: SYCL group functions and algorithms must be encountered
            in converged control flow. You may need to adjust the code.
            */
            /*
...
#pragma unroll
    for (int j = 0; j < mmq_x; j += nwarps) {
        const auto col_dst = col_dst_0 + j + item_ct1.get_local_id(1);
...
template<typename scalar_t, bool need_check> static void
#if defined(USE_ROCM)
