...
template <typename T>
__dpct_inline__ sycl::float2 __s22float2(T v);

template <typename T>
__dpct_inline__ T __float2s(float v);

template <typename T>
__dpct_inline__ T __float22s2_rn(sycl::float2 v);
...
template <>
__dpct_inline__ sycl::half __float2s(float v) {
  return sycl::vec<float, 1>(v)
      .convert<sycl::half, sycl::rounding_mode::automatic>()[0];
}

template <>
__dpct_inline__ sycl::float2 __s22float2(sycl::half2 v) {
  return v.convert<float, sycl::rounding_mode::automatic>();
}

template <>
__dpct_inline__ sycl::half2 __float22s2_rn(sycl::float2 v) {
  return v.convert<sycl::half, sycl::rounding_mode::rte>();
}
...
template <>
__dpct_inline__ __hip_bfloat16 __float2s(float v) {
  return sycl::ext::oneapi::bfloat16(v);
}

template <>
__dpct_inline__ sycl::float2 __s22float2(__hip_bfloat162 v) {
  return sycl::float2((v)[0], (v)[1]);
}

template <>
__dpct_inline__ __hip_bfloat162 __float22s2_rn(sycl::float2 v) {
  return sycl::marray<sycl::ext::oneapi::bfloat16, 2>(v[0], v[1]);
}

template <typename T>
__dpct_inline__ T loadnt(T* addr) {
  return __builtin_nontemporal_load(addr);
}

__dpct_inline__ sycl::float4 load_ntmprl(const sycl::float4* addr) {
...
void LLGemm1_kernel(const scalar_t* in_a, const scalar_t* in_b,
                               scalar_t* out_c, const int K,
                               const sycl::nd_item<3> &item_ct1, float *red_smem) {
  using scalar2_t = typename scalar2<scalar_t>::type;
  auto af4 = reinterpret_cast<const sycl::float4*>(in_a);
  auto bf4 = reinterpret_cast<const scalar2_t*>(in_b);
  auto c = reinterpret_cast<scalar2_t*>(out_c);

  const int row_addr = item_ct1.get_group(2) * NUM_A_ROWS_PER_BLOCK * K / 8;
  const int threadid = item_ct1.get_local_id(2);
  const int warp = threadIdx.x / WARP_SIZE;
  const int lane = threadIdx.x % WARP_SIZE;
  const int num_warps = blockDim.x / WARP_SIZE;
  const int qwarpid = threadid / 16;
  const int qthreadid = threadid % 16;
  sycl::float4 rowA_elem4[NUM_A_ROWS_PER_BLOCK];
...
#pragma unroll
    ...
    if (lane % 32 == 0) {
      ...
      c[item_ct1.get_group(2) * NUM_A_ROWS_PER_BLOCK / 2 + qwarpid / 2] = oval;
    }
  }
}
...
template <typename scalar_t, int THRDS, int YTILE, int WvPrGrp, int A_CHUNK,
          int UNRL, int N>
void wvSplitK_hf_sml_(const int K, const int M, const scalar_t* B,
                                 const scalar_t* __restrict__ A, scalar_t* C,
                                 const int _WvPrGrp, const int CuCount) {
  UNREACHABLE_CODE
}
...
template <typename scalar_t, int THRDS, int YTILE, int WvPrGrp, int A_CHUNK,
          int UNRL, int N>
void wvSplitK_hf_(const int K, const int M, const scalar_t* B,
                             const scalar_t* __restrict__ A, scalar_t* C,
                             const int _WvPrGrp, const int CuCount) {
  UNREACHABLE_CODE
}
...
template <typename scalar_t, int THRDS, int YTILE, int WvPrGrp, int A_CHUNK,
          int UNRL, int N>
void wvSplitK_hf_big_(const int K, const int M, const scalar_t* B,
                                 const scalar_t* __restrict__ A, scalar_t* C,
                                 const int _WvPrGrp, const int CuCount) {
  UNREACHABLE_CODE
}
...
template <typename scalar_t, typename fp8_t, int THRDS, int YTILE, int WvPrGrp,
          int A_CHUNK, int UNRL, int N>
void wvSplitKQ_hf_sml_(const int K, const int Kp, const int M,
...
template <typename scalar_t, typename fp8_t, int THRDS, int YTILE, int WvPrGrp,
          int A_CHUNK, int UNRL, int N>
void wvSplitKQ_hf_(const int K, const int Kp, const int M,
                              const fp8_t* B, const fp8_t* __restrict__ A,
                              scalar_t* C, const float* __restrict__ s_A,
                              const float* __restrict__ s_B, const int _WvPrGrp,
                              const int CuCount) {
