...
#include <algorithm>
#include "../attention/dtype_fp8.dp.hpp"
#include "../quantization/fp8/amd/quant_utils.dp.hpp"
...
void paged_attention_ll4mi_QKV_mfma16_kernel(
    const scalar_t* __restrict__ q,         // [num_seqs, num_heads, head_size]
    const cache_t* __restrict__ k_cache,    // [num_blocks, num_kv_heads, head_size/x, block_size, x]
    const cache_t* __restrict__ v_cache,    // [num_blocks, num_kv_heads, head_size, block_size]
    const int num_kv_heads,
...
void paged_attention_ll4mi_reduce_kernel(
    OUTT* __restrict__ out,                // [num_seqs, num_heads, head_size]
...
#define LAUNCH_CUSTOM_ATTENTION_MFMA16(GQA_RATIO)                              \
  stream->submit([&](sycl::handler& cgh) {                                     \
    auto query_ptr_ct0 = query_ptr;                                            \
    auto key_cache_ptr_ct1 = key_cache_ptr;                                    \
    auto value_cache_ptr_ct2 = value_cache_ptr;                                \
    auto num_kv_heads_ct3 = num_kv_heads;                                      \
    auto scale_ct4 = scale;                                                    \
    auto block_tables_ptr_ct5 = block_tables_ptr;                              \
    auto context_lens_ptr_ct6 = context_lens_ptr;                              \
    auto query_start_loc_ptr_ct7 = query_start_loc_ptr;                        \
    ...
    auto exp_sums_ptr_ct13 = exp_sums_ptr;                                     \
    auto max_logits_ptr_ct14 = max_logits_ptr;                                 \
    auto tmp_out_ptr_ct15 = tmp_out_ptr;                                       \
    auto out_ptr_ct16 = out_ptr;                                               \
    auto max_ctx_blocks_ct17 = max_ctx_blocks;                                 \
    auto k_scale_ptr_ct18 = k_scale_ptr;                                       \
    auto v_scale_ptr_ct19 = v_scale_ptr;                                       \
                                                                               \
    cgh.parallel_for(                                                          \
        sycl::nd_range<3>(grid * block, block),                                \
        [=](sycl::nd_item<3> item_ct1) {                                       \
          paged_attention_ll4mi_QKV_mfma16_kernel<T, KVT, KV_DTYPE, OUTT,      \
                                                  BLOCK_SIZE, HEAD_SIZE, NTHR, \
                                                  ALIBI_ENABLED, GQA_RATIO>(   \
              query_ptr_ct0, key_cache_ptr_ct1, value_cache_ptr_ct2,           \
              num_kv_heads_ct3, scale_ct4, block_tables_ptr_ct5,               \
              context_lens_ptr_ct6, query_start_loc_ptr_ct7,                   \
              max_num_blocks_per_seq_ct8, alibi_slopes_ptr_ct9, q_stride_ct10, \
              kv_block_stride_ct11, kv_head_stride_ct12, exp_sums_ptr_ct13,    \
              max_logits_ptr_ct14, tmp_out_ptr_ct15, out_ptr_ct16,             \
              max_ctx_blocks_ct17, k_scale_ptr_ct18, v_scale_ptr_ct19);        \
        });                                                                    \
  });

/*
DPCT1049:32: The work-group size passed to the SYCL kernel may exceed the limit.
To get the device limit, query info::device::max_work_group_size. Adjust the
work-group size if needed.
*/
#define LAUNCH_CUSTOM_ATTENTION_MFMA4(GQA_RATIO)                               \
  stream->submit([&](sycl::handler& cgh) {                                     \
    ...
    cgh.parallel_for(                                                          \
        sycl::nd_range<3>(grid * block, block),                                \
        [=](sycl::nd_item<3> item_ct1) {                                       \
          paged_attention_ll4mi_QKV_mfma4_kernel<T, KVT, KV_DTYPE, OUTT,       \
                                                 BLOCK_SIZE, HEAD_SIZE, NTHR,  \
                                                 ALIBI_ENABLED, GQA_RATIO>(    \
              query_ptr_ct0, key_cache_ptr_ct1, value_cache_ptr_ct2,           \
              num_kv_heads_ct3, scale_ct4, block_tables_ptr_ct5,               \
...
/*
DPCT1049:34: The work-group size passed to the SYCL kernel may exceed the limit.
To get the device limit, query info::device::max_work_group_size. Adjust the
work-group size if needed.
*/
#define LAUNCH_CUSTOM_REDUCTION(NPAR_LOOPS)                                   \
  stream->submit([&](sycl::handler& cgh) {                                    \
    ...
    auto query_start_loc_ptr_ct5 = query_start_loc_ptr;                       \
    auto max_num_partitions_ct6 = max_num_partitions;                         \
    auto fp8_out_scale_ptr_ct7 = fp8_out_scale_ptr;                           \
                                                                              \
    cgh.parallel_for(                                                         \
        sycl::nd_range<3>(reduce_grid * reduce_block, reduce_block),          \
        [=](sycl::nd_item<3> item_ct1) {                                      \
          paged_attention_ll4mi_reduce_kernel<T, OUTT, HEAD_SIZE, HEAD_SIZE,  \
                                              PARTITION_SIZE, NPAR_LOOPS>(    \
              out_ptr_ct0, exp_sums_ptr_ct1, max_logits_ptr_ct2,              \
              tmp_out_ptr_ct3, context_lens_ptr_ct4, query_start_loc_ptr_ct5, \
              max_num_partitions_ct6, fp8_out_scale_ptr_ct7);                 \
        });                                                                   \
  });
...
void paged_attention_custom_launcher(
  ...
  dpct::dim3 grid(num_seqs, max_num_partitions, num_kv_heads);
  dpct::dim3 block(NTHR);
  ...
  dpct::dim3 reduce_grid(num_heads, num_seqs);
  dpct::dim3 reduce_block(head_size);
