...
template <>
inline sycl::float2 mul(sycl::float2 a, sycl::float2 b) {
  sycl::float2 c;
  c.x() = a.x() * b.x();
  c.y() = a.y() * b.y();
  return c;
...
template <>
inline sycl::float2 mul(float a, sycl::float2 b) {
  sycl::float2 c;
  c.x() = a * b.x();
  c.y() = a * b.y();
  return c;
...
template <>
inline sycl::float4 mul(sycl::float4 a, sycl::float4 b) {
  sycl::float4 c;
  c.x() = a.x() * b.x();
  c.y() = a.y() * b.y();
  c.z() = a.z() * b.z();
...
template <>
inline sycl::float4 mul(float a, sycl::float4 b) {
  sycl::float4 c;
  c.x() = a * b.x();
  c.y() = a * b.y();
  c.z() = a * b.z();
...
template <>
inline float sum(sycl::float2 v) {
  return v.x() + v.y();
}

template <>
inline float sum(sycl::float4 v) {
...
inline void from_float(sycl::float2& dst, sycl::float2 src) { dst = src; }

inline void from_float(sycl::float4& dst, sycl::float4 src) { dst = src; }

// From float to float.
inline float to_float(float u) { return u; }

inline sycl::float2 to_float(sycl::float2 u) { return u; }

inline sycl::float4 to_float(sycl::float4 u) { return u; }

inline Float4_ to_float(Float4_ u) { return u; }

inline Float8_ to_float(Float8_ u) { return u; }

// Zero-out a variable.
inline void zero(float& dst) { dst = 0.f; }
