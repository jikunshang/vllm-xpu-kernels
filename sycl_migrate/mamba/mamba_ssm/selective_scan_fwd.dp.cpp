...
void selective_scan_fwd_kernel(SSMParamsBase params,
    ...
    const int dim_id = item_ct1.get_group(1);
    ...
    for (int chunk = 0; chunk < n_chunks; ++chunk) {
        ...
        /*
        DPCT1118:243: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
        */
        /*
        DPCT1065:401: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
        */
        item_ct1.barrier();
        #pragma unroll
        for (int r = 0; r < kNRows; ++r) {
            if constexpr (!kDirectIO) {
                /*
                DPCT1118:246: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
                */
                /*
                DPCT1065:404: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
                */
                if (r > 0) { item_ct1.barrier(); }
            }
            load_input<Ktraits>(u + r * params.u_d_stride, u_vals[r], smem_load, seqlen - chunk * kChunkSize);
            /*
            DPCT1118:247: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
            */
            /*
            DPCT1065:405: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
            */
            if constexpr (!kDirectIO) { item_ct1.barrier(); }
        ...
        /*
        DPCT1118:244: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
        */
        /*
        DPCT1065:402: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
        */
        item_ct1.barrier();
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                /*
                DPCT1118:248: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
                */
                /*
                DPCT1065:406: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
                */
                if (r > 0) { item_ct1.barrier(); }  // Scan could be using the same smem
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    if (seqlen % (kNItems * kNThreads) != 0) {  // So that the last state is correct
                        if (item_ct1.get_local_id(2) * kNItems + i >= seqlen - chunk * kChunkSize) {
                            thread_data[i] = sycl::float2(1.f, 0.f);
                        }
                    }
                }
                // Initialize running total

                scan_t running_prefix = chunk > 0 ? smem_running_prefix[state_idx + r * MAX_DSTATE] : sycl::float2(1.0, has_initial_state ? float(ssm_states[state_idx]): 0.0);
        ...
        /*
        DPCT1118:245: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
        */
        /*
        DPCT1065:403: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
        */
        item_ct1.barrier();
        #pragma unroll
        for (int r = 0; r < kNRows; ++r) {
            if constexpr (!kDirectIO) {
                /*
                DPCT1118:249: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
                */
                /*
                DPCT1065:407: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
                */
                if (r > 0) { item_ct1.barrier(); }
            }
        ...
        if constexpr (kHasZ) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                /*
                DPCT1118:250: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
                */
                /*
                DPCT1065:408: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
                */
                item_ct1.barrier();
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    out_vals[r][i] *= z_val / (1 + sycl::native::exp(-z_val));
                }
                /*
                DPCT1118:251: SYCL group functions and algorithms must be encountered in converged control flow. You may need to adjust the code.
                */
                /*
                DPCT1065:409: Consider replacing sycl::nd_item::barrier() with sycl::nd_item::barrier(sycl::access::fence_space::local_space) for better performance if there is no access to global memory.
                */
                item_ct1.barrier();
...
void selective_scan_fwd_launch(SSMParamsBase &params, dpct::queue_ptr stream) {
    ...
    /*
    DPCT1038:252: When the kernel function name is used as a macro argument, the migration result may be incorrect. You need to verify the definition of the macro.
    */
    BOOL_SWITCH(params.seqlen % (kNThreads * kNItems) == 0, kIsEvenLen, [&] {
        BOOL_SWITCH(params.z_ptr != nullptr , kHasZ, [&] {
            BOOL_SWITCH(params.query_start_loc_ptr != nullptr , kVarlen, [&] {
                ...
                dpct::dim3 grid(params.batch, params.dim / kNRows);
                ...
                dpct::kernel_launcher::launch(kernel, grid, Ktraits::kNThreads, kSmemSize, stream, params);
...
void selective_scan_fwd_cuda(SSMParamsBase &params, dpct::queue_ptr stream) {
