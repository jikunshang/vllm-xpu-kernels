...
void dummy_kernel() {
  ...
  /*
  DPCT1008:274: __nanosleep function is not defined in SYCL. This is a
...
/*
DPCT1032:275: A different random number generator is used. You may need to
adjust the code.
*/
void init_rand(
  ...
  for (int idx = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
    ...
    for (int i = 0; i < nRanks; i++) {
      /*
      DPCT1105:276: The mcg59 random number generator is used. The subsequence
...
template <typename T>
/*
DPCT1032:277: A different random number generator is used. You may need to
adjust the code.
*/
void gen_data(
    dpct::rng::device::rng_generator<oneapi::mkl::rng::device::mcg59<1>>* state,
...
void run(int myRank, int nRanks, ncclComm_t& comm, int threads, int block_limit,
  ...
  /*
  DPCT1025:278: The SYCL queue is created ignoring the flag and priority
  options.
  */
  CUDACHECK(
  ...
  CUDACHECK(
      DPCT_CHECK_ERROR(self_data_copy = (T*)sycl::malloc_device(
                           data_size * sizeof(T), dpct::get_in_order_queue())));
  /*
  DPCT1030:279: SYCL currently does not support inter-process communication
  ...
  */
  ...
  for (int i = 0; i < nRanks; i++) {
    if (i == myRank)
      ...
      /*
      DPCT1030:280: SYCL currently does not support inter-process communication
      (IPC) operations. You may need to rewrite the code.
      */
  ...
  /*
  DPCT1032:281: A different random number generator is used. You may need to
  adjust the code.
  */
  dpct::rng::device::rng_generator<oneapi::mkl::rng::device::mcg59<1>>* states;
  /*
  DPCT1032:282: A different random number generator is used. You may need to
  adjust the code.
  */
  ...
  */
  {
    ...
    stream->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 108) *
                                           ...
                                           sycl::range<3>(1, 1, 1024)),
                         [=](sycl::nd_item<3> item_ct1) {
                           gen_data<T>(states, self_data, ground_truth, myRank,
                                       nRanks, data_size, item_ct1);
                         });
  }
  /*
  DPCT1124:283: cudaMemcpyAsync is migrated to asynchronous memcpy API. While
  ...
  */
  ...
  if (performance_test) {
    ...
    /*
    DPCT1024:284: The original code returned the error code that was further
    ...
    */
    ...
    /*
    DPCT1024:286: The original code returned the error code that was further
    consumed by the program logic. This original code was replaced with 0. You
    may need to rewrite the program logic consuming the error code.
    */
    CUDACHECK(DPCT_CHECK_ERROR(*start = stream->ext_oneapi_submit_barrier()));
    ...
    /*
    DPCT1024:287: The original code returned the error code that was further
    consumed by the program logic. This original code was replaced with 0. You
    may need to rewrite the program logic consuming the error code.
    */
    CUDACHECK(DPCT_CHECK_ERROR(*stop = stream->ext_oneapi_submit_barrier()));
    ...
    */
    {
      ...
      stream->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 108) *
                                             ...
                                             sycl::range<3>(1, 1, 1024)),
                           [=](sycl::nd_item<3> item_ct1) {
                             convert_data<T>(self_data, result, nccl_result,
      ...
      */
      {
        ...
        stream->parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, 108) *
                                               ...
                                               sycl::range<3>(1, 1, 1024)),
                             [=](sycl::nd_item<3> item_ct1) {
                               convert_data<T>(self_data_copy, result,
                                               nccl_result, my_result,
                                               data_size, item_ct1);
                             });
      }
      CUDACHECK(DPCT_CHECK_ERROR(stream->wait()));
...
int main(int argc, char** argv) try {
  ...
  /*
  DPCT1093:290: The "myRank" device may be not the one intended for use. Adjust
  ...
  */
  ...
  /*
  DPCT1026:288: The call to cudaProfilerStart was removed because SYCL currently
...
// }
  ...
  /*
  DPCT1026:289: The call to cudaProfilerStop was removed because SYCL currently
  does not support this function. Remove the profiler API will not impact the
  outcome.
  */
